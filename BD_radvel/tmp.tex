...
the RV by location the peak, (4)~repeat 1--3 for 100 times, and computes the standard
deviation of the 100 RV values. This should provide a good estimate of
uncertainties in RV cased by the uncertainties in the flux levels.
%The two consecutive measurements of RVs (from same nights) are used to
...
separations and the average uncertainty ($\sim0.56\,\kmps$) in the RV
measurements. 
...
\caption{The binary detection probability (solid) as a function of the semimajor
  ...
  $~0.56\,\kmps$. The probability remains fairly
...
time-scale. The shifts in the RV values in measured in the two
consecutive spectra are found to be between $0.02\pm0.10$ (USco~128)
and  $2.04\pm1.67\,\kmps$ (USco~66), which is much smaller than the
expected RV variation based on the models of \citet{palla:2005};
hence, found little indication of the pulsations. 
...
$24^{+16}_{-13}$~per~cent for the binary separation $a<0.1$~au.  We found
...
a triple system with one of object located within 0.1~au from the
...
in service mode. RK is grateful for Rob Jeffries for helpful
suggestions on the data analysis presented in this paper. RK also
thank Matteww Bate and Tim Naylor for providing us helpful comments on

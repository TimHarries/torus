...
\textcolor{red}{
As we will demonstrate in the following section
(Section~\ref{sub:Radial-velocities}), the systematic error may be
ignored based on the result of the $\chi^{2}$ probability analysis we
performed without the introduction of an assumed systematic error.
}
A typical
...
and the same instrument set up as for our main targets.  The S/N of
...
\textcolor{red}{
  ...
  dominates in the uncertainty in the value of
...
absolute\textcolor{red}{/heliocentric} radial velocities, but only the relative
...
\textcolor{red}{
The assertion is supported by the re-analysis of BD RVs, obtained by
\citet{joergens:2006} who used the same instruments setup
...
\caption{Histogram of the $\chi^2$ probability ($p$) for fitting the
  ...
  histogram is reasonable \textcolor{red}{(c.f.~Section~\ref{sub:Radial-velocities})},
...
\textcolor{red}{
It is possible that out binary fraction is an overestimate since we
are biased to brighter objects. However the lack of a double-lined

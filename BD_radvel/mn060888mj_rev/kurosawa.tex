...
\textcolor{red}{
As we will find out in the following section
(Section~\ref{sub:Radial-velocities}), the systematic error may be
ignored based on result of the $\chi^{2}$ probability analysis we
performed without the introduction of assumed systematic error.
}
A typical
...
velocities in column~4 and 6 includes the uncertainty in the
heliocentric RV of the template star, but $\sigma_{\mathrm{RRV}}$
do not.
}
}
...
\textcolor{red}{
  ...
  by combining the uncertainty ($0.4~\kmps$) in the heliocentric RV of
  the template star HD~74497 (G3V) and the uncertainty in the
  relative RV of HD~140538 ($0.02~\kmps$), measured with respect to
  the template star.  Clearly, the former dominates in the uncertainty
  in the value of $\mathrm{RV_{HD\,140538}}$ shown above. The
  uncertainty in the heliocentric RV of the template is rather large
  since it was determined from a few photospheric lines.
}

\textcolor{red}{
  The result of the heliocentric RV measurements (from two
  epochs for each object) is summarised in Table 2 along with their
  uncertainties.  The uncertainties listed with the
  absolute/heliocentric RV values in the 4-th and 6-th columns in the
  table include the uncertainty of the absolute/heliocentric RV of
  the template star. On the other hand, the uncertainties in the
  `relative' radial velocities ($\sigma_{\mathrm{RRV}}$) listed in the 5-th
  column do not include the uncertainty of the template star. It is
  $\sigma_{\mathrm{RRV}}$ that is relevant for the identification of RV variable
  objects.  
}
...
curve. 
\textcolor{red}{
To quantify this point, we have computed the
...
from the left $\left(0 < -\log_{10}p < 2 \right)$ the
histogram since the objects in the second bin from the right
$\left(2.5 < -\log_{10}p < 3 \right)$ are `potentially'
RV-variable object, although we have flagged them as the RV-constant
...
systematic error may not be necessary in this analysis. 
}

\textcolor{red}{
There are two possible
source of systematic errors: (1)~the RV shifts caused by surface/atmospheric
...
and incomplete), hence because of a possible bias towards the brighter
objects.
}

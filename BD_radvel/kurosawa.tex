...
found 2 out of 17 objects (or $12^{+13}_{-10}$~per~cent by fraction)
...
cross-corelation curves. The RV dispersion of the objects in UScoOB is
found to be very similar to that of the BDs and VLM stars in
Chamaeleon~I (Cha~I) previously studied by others. We also found the
distribution of the rotational velocities ($v\,\sin i$) of the UScoOB
objects is good agreement with that of Cha~I from earlier studies.
...
epochs separated by several (between 4 and 33~d). For each object at a
...
\caption{Summary of known properties of the targets from literature:
...
(excluding the non-member USco~121). 
...
curve. This assures that our uncertainties in RV values are
reasonable, and an additional systematic error may not be necessary in
this analysis. According
to the figure, there are four objects which shows significant RV
...
list our final binary/multiple candidate later in
Section~\ref{sec:Binary-fraction}. 
...
\caption{The histogram of the $\chi^2$ probability ($p$) for fitting the
  ...
  which appear in the right-most $\log{p}$ bin in the histogram. There
  are four objects which satisfies this condition. The
  expected distribution (dashed) for constant objects based on the RV
...
a non-member). The gaussian fit (dashed line) of the radial velociity
...
\caption{Histogram of the rotational velocities (right) of 14 UScoOB BD and VLM
objects listed in Table~\ref{tab:summary_results} (excluding USco~121,
a non-member).  The log-normal fit (dash-dot) of the
...
analysis, we find the final binary/multiple fraction along with the
...
\ref{tab:summary_results}).  The probability will be calculated based
...
(5) repeat 1--4 for a range of $a$ between $10^{-3}$~and 10~au, and
...
increase if we used larger time separations in our observation. 
...
separations and the average uncertainty ($~0.5\,\kmps$) in the RV
measurements. 
...
\caption{The binary detection probability (solid) as a function of the semimajor
  ...
  using the time separation ($\Delta t$) of the two-epoch observation
  used in the observation (c.f.~Table~\ref{tab:summary_results}) and
  ...
  The binary separation distribution (dash) similar to that of
  \citet{maxted:2005} and \citet{basri:2006} are shown also for a
  comparison. The distribution is a gaussinan in logarithmic
  space, centred at $\log{a}=10$, and truncated at $a=15$~au. See text
  for a detail.}  
...
Apparently, the underestimated uncertainties could have been caused by
neglecting a systematic error. In Section~\ref{sub:Radial-velocities}
(see Fig.~\ref{fig:chisq-test}), we found the histogram of the
$\chi^{2}$ probability $p$, for the RV data fit assuming constant RV,
is reasonably matches with the expected distribution.  This lead us to
...
The observed binary fraction is expected to be very sensitive to the
assumed size of the systematic error since the RV shifts and the
systematic errors are in same order of magnitude.  This point is
demonstrated further in Fig.~\ref{fig:adding_sys_err} which shows the
the number of observed binaries (non-constant RV objects) plotted
against the size of the additional systematic error.  The number of
detections decreases rapidly as $\sigma_{\mathrm{sys}}$ increases.
The number remains constant at 2 for a relatively wide range of the
systematic errors i.e.  $0.05\,\kmps < \sigma_{\mathrm{sys}} < 0.275\,
\kmps$.  No binary is detected for $\sigma_{\mathrm{sys}} > \sim
0.375\, \kmps$.  We have computed the $\chi^2$ probabilities for the
RV constant as done earlier (Fig~\ref{fig:chisq-test}) along with the
expected distribution of ($-\log{p}$) with an additional
$\sigma_{\mathrm{sys}}$ introduced in the analysis. We find that the
case with $\sigma_{\mathrm{sys}}=0.1\,\kmps$ gives a better agreement
with the histogram of $-\log{p}$ to the expected
distribution (not shown here), compared to the case with
$\sigma_{\mathrm{sys}}=0\,\kmps$. In the following analysis, we will
adopt $\sigma_{\mathrm{sys}}=0.1\,\kmps$. This means that even if our
systematic error is slightly underestimated, the number of detection
remains the same according to Fig.~\ref{fig:adding_sys_err}.

We have re-analysed the number of observed binaries
(Fig~\ref{fig:chisq-test}), the detection probability
(Fig.~\ref{fig:detect_prob}), and the expected binary fraction as done
earlier but this time with the additional systematic error
$\sigma_{\mathrm{sys}}=0.1\,\kmps$.  The result is summarised in
Table~\ref{tab:binary_fraction}.  
Although the small number statistics makes us difficult to withdraw a
firm conclusion on the binary fraction from our data, we find the case
with $\sigma_{\mathrm{sys}} = 0.1\, \kmps$ has a better agreement with
the expected binary fraction (c.f.~Table~\ref{tab:binary_fraction}).
Our tentative conclusion of the binary fraction found in this analysis
is $f_{\mathrm{sys}}=12^{+13}_{-10}$~per~cent for $a<0.12$~au.  The
large uncertainties in this values is due to small number
statistics. The expected binary fraction
$f_{\mathrm{exp}}=2.9$~per~cent is still within the uncertainty of our
binary fraction.  The two objects identified as 
binary/multiple candidates are GY~310 and USco~104.
...
\caption{The number of binaries/multiples (non-constant RV objects)
  ...
  in RV measurement. The number of detections decreases rapidly from 4
  to 2 as $\sigma_{\mathrm{sys}}=0.1\,\kmps$, which may be caused by
  from the instability of the autoguiding of the telescope, is added to
  the uncertainties estimated in
  Table~\ref{tab:summary_results}. The number of the detections
...
\caption{Summary of observed binary fraction ($f_{\mathrm{obs}}$) compared with the
  expected binary fraction ($f_{\mathrm{exp}}$) with and without a
...
We found 2 (GY~310 and USco~104) out of 17 objects as our final
binary/multiple candidates. This corresponds to the binary fraction of
$12^{+13}_{-10}$~per~cent for the binary separation $a<0.12$~au, which
is larger than the expected binary fraction of 2.9~per~cent, but it is
still within the uncertainties of the observed value.  The expected
binary fraction was computed by using a similar binary separation
models used by \citet{maxted:2005} and \citet{basri:2006}.  We found
that the final estimate of the binary fraction is very sensitive to
the assumed size of systematic error. If the systematic error was
ignored, we would have found 4 binary/multiple candidates.
...
($a>4.0$~au), and are well above our sensitivity limit of
0.12~au. Hence, it is not surprising that we did not find any of them
as a multiple, and it also indicates that any of them is unlikely to
have a component of a triple system within 0.12~au from their primaries. 
On the other hand, we found USco~104 as a binary candidate, but this

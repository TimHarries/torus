...
\makeatother
\begin{document}
\title[Radial and rotational velocities of young brown dwarfs candidates  in the Upper Sco and $\rho$~Oph]{Radial and rotational velocities of young brown dwarfs and very low-mass stars in Upper Scorpius OB association and $\rho$~Ophiuchi cloud core}
...
We present the results of a radial velocity (RV) survey for 15 brown
...
candidates for binary/multiple systems in the sample. We found no
spectroscopic binary in our sample, based on the shape of cross-corelation
curves. We found 9 out of 18 (50 per cent) in our sample show a significant
...
to be confirmed by follow-up spectroscopic monitoring. Our RV-shift
tests successfully detected the recently confirmed 3 binaries in UScoOB
...
measured the rotational velocities ($v\,\sin i$) of the samples,
...
is also similar to that of the Cha~I population. 
...
Most stars are member of binary systems; hence, it is important for
a star formation theory to be able to predict the binary fraction,
period distribution, and mass-ratio distribution of newly born stellar
objects across a wide range mass spectrum. The study of the individual
binary system is the only direct mean to determine the fundamental
...
lack of wide binaries were found in the field T-dwarf study \citep{burgasser:2003},
...
by using similar imaging technique.. 

Using Monte Carlo simulation techniques, the data from radial velocity
surveys available on literatures, and carefully considering the sensitivity
...
readers are refer to an excellent article on multiplicity studies
...
of less than 1~au are not resolved by current imaging technique,
...
is 30~au \citet{duquennoy:1991}. The pioneer works on the RV studies
of BDs and VLMs are presented by \citet{guenther:2003}, \cite{kenyon:2005}
and \citet{joergens:2006} who found a sevral binary candidates among
them; however, the orbital parameters and masses of binaries remains
unknown because the follow-up spectroscopic monitoring is lacking
or still being taking places. In addition to the follow-up observations,
the number of BDs and VLM star binary candidates should be increased
to have a better statistics on the short period binary parameters. 
...
object can be formed in the process of the photo-evaporation \citep{whitworth:2004}.
The outer layers of a pre-stellar core ($\sim0.2\,\mathrm{M_{\odot}}$)
removed by the strong radiation pressure from the nearby massive O
and B stars before the accretion onto the protostar at core centre
occurs. Apparently, this is more relevant to the high-mass star forming
clouds. 
...
mechanisms, one would expect to find binaries with 1--10~au separations
without also finding many with separation $<1$~au. If the absence/rarity
...
the interactions that shorten the periods. 
...
found in this paper by spectroscopically monitoring them in different
...
fraction indicated by our RV survey in Section~\ref{sec:Binary-frequency}.
Finally, conclusions will be given in Section~\ref{sec:Conclusions}.
...
Our sample consist of 18 young very low-mass objects previously identified
in the star forming regions: 15 objects in the Upper Scorpius OB association
($\mathrm{d\approx145\, pc}$, \citealt{dezeeuw:1999}) from the list
of \citet{ardila:2000} and 3 objects in the $\rho$~Ophiuchi cloud
core ($\mathrm{d\approx150\, pc}$, \citealt{dezeeuw:1997}) from
 \citet{luhman:1999}. The spectral type of the objects spreads between
...
The sample is not complete, and the selection is mainly based on the
brightness and the observability. The basic known properties of the
targets based on literatures is summarised in Table~\ref{tab:literatures}.
...
were carried out between April 5, 2004 and May 15, 2005 in the service
mode. For each object, the spectra were obtained at two different
epochs separated by 4--33~d to increase the chance of detectability
for multiplicities. For each object at a given night, two separate
spectra are obtained consecutively. This will allow us to derive more
reliable uncertainty estimates in the RV values of our targets (c.f.~\citealt{joergens:2006}).
The data was obtained using the red arm of UVES spectrograph with
...
were used. Unfortnately, the litheum lines at $\lambda\lambda6707.76$,$6707.93$
just fell shortward of the minimum wavelength. The slit width and
length of 1'' and 12'' are used respectively with a typical seeing
of 0.8''. 

The data were reduced by the standard ESO pipeline procedures for
...
ray hits. Further, they were flattened, optimally extracted, and finally
...
\caption{Summary of known properties of the targers from literatures: a.~\citet{luhman:1999} (original list for $\rho$~Oph), b.~\citet{ardila:2000} (original list for UScoOB),  c.~\citet{wilking:1999}, d.~\citet{muzerolle:2003}, e.~\citet{kraus:2005}, and f.~\citet{mohanty:2005}. }
...
\caption{Relative radial velocities (RVs) of objects measured in two different epochs. The vertical axes indicate  the amount of deviation ($\Delta \mathrm{RV}$) from the `average' radial velocitiy  ($\overline{\mathrm{RV}}$) in Table~\ref{tab:summary_results}), and the horizontal axes indicate the time of the observation in modified Julian date (MJD). The objects are considered to have a non-constant  RV when the error bars of two data points do not overlap each other. The non-constant RV objects are considered as binary/multiple candidates.}
...
The radial velocities of each object are determined by using the cross-correlation
function of object spectra with that of a template star which has
a spectra type similar to those of our objects. By visual inspections,
...
lines, the defects and the fringes (in near infrared) of CCDs, The
...
which is fitted by the combination of two gaussian functions sharing
their centres and a constant term. LHS~49 (Proxima Cen, M5.5) is
...
($\mathrm{RV_{LHS49}=}-22.6\pm0.5\,\kmps$) will be used for a consistency. 
...
is the uncertainties of RVs with respect to the template star. The
two consecutive measurements of RVs (from same nights) are used to
find the RV of the night, and their uncertainties are obtained from
...
two epochs are also given in the same table for a reference. The $\overline{\mathrm{RV}}$
...
for the values for USco~66 and USco~75. This difference is not surprising
...
to aid the identifications of multiplicity candidates. Note that in
...
velecities (with respect to a template). We choose a object as a candidate
...
caused of the difference is that the uncertainties in RVs may be underestimated.
The surface activities of the objects may also contribute noise in
...
According to the hydrodynamical calculation of a low-mass star forming
...
\caption{Histogram of the average heliocentric radial velocities (left) and the rotational velocities (right) of 14 UScoOB BD and VLM objects listed in Table~\ref{tab:summary_results} (excluding USco~121, non-member). The gaussian fit (dash) of the radial velociity distribution gives the standard deviation of  $1.2\,\kmps$ and the peak position of $-5.9\,\kmps$.  The log-normal fit (dash-dot) of  the rotational velocity distribution gives  the standard deviation $27.8\,\kmps$ and the peak poistion $16.9\,\kmps$.}
...
The rotational velocities of the objects are determined by measuring
the widths of the cross correlation curves of the target spectra against
a templates spectra from an object which is known to have a very small
rotational velocity. The line broadening of the targets are assumed
to be dominated by rotational broadening. As in the cases for the
radial velocity measurements, LHS~49 is chosen for the template.
Using its known period ($P\approx83\,\mathrm{d}$, \citealt{benedict:1998})
...
with added rotation (convolved with a given $v\,\sin i$), as similarly
done by e.g.~\citet{tinney:1998}, \citet{mohanty:2003} and \citet{white:2003}.
The linear limb-darkening law with a solar-like parameter ($\epsilon=0.6$)
...
obtained at different epochs. As similary done for the RV measurements,
the mean and the standard deviation of the mean are used as the final
rotational velocity and its uncertainty. The final results are placed
...
the peak position at $16.9\,\kmps$ and the standard deviation $\sigma=27.8\,\kmps$.
...
very small number of samples. A similar fit was applied to the $v\,\sin i$
distribution of 14 T~Tauri stars in Cha~I using the data of \citet{joergens:2001},
and we found the peak at $17.0\,\kmps$ and the standard deviation
...
cent of) samples are considered as binary/multiple candidates. The
...
UVES echelle spectroscopy at VLT. The amount of the shifts in RVs
in time is used to identify the candidates for multiplicity. 

We found 9 out of 18 objects (50~per~cent) are possible candidates
...
This work is supported by PPARC rolling grant PP/C501609/1. We thank
the staff of VLT of the ESO for the observations carried out in the
service mode. 

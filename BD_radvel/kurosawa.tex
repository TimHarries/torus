...
were carried out between 2004 April 5 and 2005 (XXX?)  May 15 in  service
...
\caption{Relative radial velocities (RVs) of objects measured in two different epochs. The vertical axes indicate  the amount of deviation ($\Delta \mathrm{RV}$) from the `average' radial velocitiy  ($\overline{\mathrm{RV}}$) in Table~\ref{tab:summary_results}), and the horizontal axes indicate the time of the observation in modified Julian date (MJD; XXX do you really mean MJD rather than heliocentric JD????). The objects are considered to have a non-constant  RV when the error bars of two data points do not overlap each other. The non-constant RV objects are considered as binary/multiple candidates.}
...
their centres and a constant term (XXXX What does this mean?). LHS~49 (Proxima Cen, M5.5) was
...
(XXX What do you need to include the uncertainty in the template
star measurement when we are looking for relative changes in the
bd RVs?) The 'average' radial velocities ($\overline{\mathrm{RV}}$) of the
...
the staff of VLT of the ESO for the observations carried out in
service mode. 

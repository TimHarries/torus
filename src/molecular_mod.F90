...
!tjh added
                 ...
                 if (writeoutput) write(*,*) "Original maxlevel ",maxlevel
                 call reduceMolecule(thisMolecule, maxlevel)
                 if (writeoutput) write(*,*) "Final maxlevel ",maxlevel
...
! 3rd) setting npops = radiative solution if radiatively dominated
     ...
     if ( (debug .and. any(isnan(matrixB))) .or. (.not.luslvOK) ) then
        ...
        if ( (debug .and. .not. any(isnan(matrixBsave))) .or. luslvOK ) then
           ...
           if(nh2 * 2.d0 * mhydrogen .gt. maxval(thismolecule%einsteinA(1:maxtrans) / ctot(1:maxtrans))) then
...
! tjh april 2019 changed sign of imagebasis(2) in expression below from - to +, and removed a +imagebasis(2) from the end

      pixelcorner = ObserverVec - ( dble(cube%nx)*0.5d0*imagebasis(1) + dble(cube%ny)*0.5d0*imagebasis(2) ) 
...
! tjh april 2019 changed sign of imagerbasis(2) in expression below from - to +
...
! tjh april 2019 changed sign of pixelbasis(2) in expression below from - to +
...
! Define a vector that points *from* a specific observer that may be rotated
           ...
           imagebasis(2) = VECTOR(0.d0,1.d0,0.d0) ! Define a vertical basis vector for the image

module obj_io_mod
! Module for read WaveFront obj mesh files.
!
! Based on:
! http://people.sc.fsu.edu/~jburkardt/f_src/obj_io/obj_io.html
!
contains
...
logical function ch_is_control ( c )
...
!
  ...
  return
end subroutine obj_face_node_print
...
!
  ...
  return
end subroutine obj_normal_vector_print
...
!
  ...
  return
end subroutine obj_node_xyz_print
subroutine obj_read ( input_filename, node_num, face_num, normal_num, &
  ...
  object_name, object_material, object_start_node, object_start_face, object_start_normal)
...
!
  ...
  integer :: objectCount, maxObjectCount
  character(len=255), optional, pointer, dimension(:), intent(out) :: object_name
  character(len=255), optional, pointer, dimension(:), intent(out) :: object_material
  character(len=255), pointer, dimension(:) :: object_name_tmp
  character(len=255), pointer, dimension(:) :: object_material_tmp
  integer(kind=4), optional, pointer, dimension(:), intent(out) :: object_start_node
  integer(kind=4), optional, pointer, dimension(:), intent(out) :: object_start_normal
  integer(kind=4), optional, pointer, dimension(:), intent(out) :: object_start_face


  parameter(maxObjectCount = 80)
  ...
  objectCount = 0
  if(present(object_name)) allocate(object_name(maxObjectCount))
  if(present(object_material)) allocate(object_material(maxObjectCount))
  if(present(object_start_node)) allocate(object_start_node(maxObjectCount))
  if(present(object_start_face)) allocate(object_start_face(maxObjectCount))
  if(present(object_start_normal)) allocate(object_start_normal(maxObjectCount))
...
!
        ...
        if(objectCount <= maxObjectCount) then
        call word_next_read ( line, word, done )
            if(present(object_name)) object_name(objectCount) = word
            if(present(object_start_node)) object_start_node(objectCount) = node+1
            if(present(object_start_face)) object_start_face(objectCount) = face+1
            if(present(object_start_normal)) object_start_normal(objectCount) = normal+1
        else if(objectCount == maxObjectCount + 1) then ! Issue warning only once.
            print *, 'WARNING in obj_read - encountered more object than given maxObjectCount.'
            print *, '  Will continue to read, but object information is lost.'
            print *, '  Increase maxObjectCount.'
        end if
...
!
        ...
        call word_next_read ( line, word, done )
        if(present(object_material)) object_material(objectCount) = word
!
!  V X Y Z
!  Geometric vertex.
!
...
!
  ...
  if(present(object_name)) then
    allocate(object_name_tmp(objectCount))
    !object_name_tmp => object_name(1:objectCount)
    object_name_tmp = object_name(1:objectCount)
    deallocate(object_name)
    object_name => object_name_tmp
  end if

  if(present(object_material)) then
    allocate(object_material_tmp(objectCount))
    !object_name_tmp => object_name(1:objectCount)
    object_material_tmp = object_material(1:objectCount)
    deallocate(object_material)
    object_material => object_material_tmp
  end if

  if(present(object_start_node)) call trim_int_array(object_start_node, objectCount)
  if(present(object_start_face)) call trim_int_array(object_start_face, objectCount)
  if(present(object_start_normal)) call trim_int_array(object_start_normal, objectCount)
...
subroutine trim_int_array(ptr, new_size)
    ...
    integer(kind=4), dimension(:), pointer :: tmp

    allocate(tmp(new_size))
    tmp = ptr(1:new_size)
    deallocate(ptr)
    ptr => tmp
end subroutine trim_int_array
...
!
  ...
  return
end subroutine obj_vertex_normal_print

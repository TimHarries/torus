...
!#endif
    ...
    if (bioPhysics) then
     ...
     if (dustPhysics.and.radiativeEquilibrium) then
        ...
        if (grid%geometry == "jaehan2d") then
           call fillDustJaehan(grid, grid%octreeRoot, dustMass)
           write(*,*) "Large dust mass ",dustMass/msol
        endif
...
!        if(.not. discardsinks .and. 0 == 1) then
        if(.not. discardsinks) then
           ...
           if (get_nptmass().ne.inputnsource) then
              call writeWarning("Number of sinks found in sph file not the same as quoted in parameters file")
           else

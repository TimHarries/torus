...
!             if(thisOctal%pressure_i(subcell) /= neighbourOctal%pressure_i(subcell)) then
  ...
  recursive subroutine checkForBoundaries2Dcylindrical(thisOctal, grid)
   ...
   type(gridtype) :: grid   
   integer :: i, subcell, neighbourSubcell
   logical :: problem = .true.
   type(VECTOR) :: rVec, locator
   type(VECTOR), parameter,dimension(4) :: directions=(/VECTOR(1.d0, 0.d0, 0.d0),VECTOR(-1.d0, 0.d0, 0.d0),&
                                                        VECTOR(0.d0, 0.d0, 1.d0),VECTOR(0.d0, 0.d0, -1.d0)/)
    ...
    do subcell = 1, thisOctal%maxChildren
       if (thisOctal%hasChild(subcell)) then
          ...
          do i = 1, thisOctal%nChildren, 1
             if (thisOctal%indexChild(i) == subcell) then
                child => thisOctal%child(i)
                call checkForBoundaries2Dcylindrical(child, grid)
                exit
             end if
          end do
       else
          do i=1, 4
             rVec = subcellCentre(thisOctal, subcell)
             locator = rvec  + directions(i) * (thisoctal%subcellsize/2.0+smallestCellSize*0.1)
             neighbouroctal => thisoctal
             thisOctal%isOnBoundary(subcell,i)=1
             if (inOctal(grid%octreeRoot, locator) .and. locator%x>0) then
                call findsubcelllocal(locator, neighbourOctal, neighbourSubcell, problem)
                if (octalOnThread(neighbourOctal, neighbourSubcell, thisOctal%mpithread(subcell))) then
                   thisOctal%isOnBoundary(subcell,i)=0
                endif
             endif
          end do
       end if
 ...
 end subroutine checkForBoundaries2Dcylindrical
...
!          n = 1.125
 !         rVec = subcellCentre(thisOctal,subcell)
  ...
  recursive subroutine restrictResiduals(thisoctal, grid, nDepth)
    use mpi
    use inputs_mod, only : smallestCellSize, gridDistanceScale
    type(GRIDTYPE) :: grid
    type(octal), pointer   :: thisoctal
    type(octal), pointer  :: child 
    integer :: i, nDepth, subcell,n, nd, nc
    integer :: nDir, neighbourSubcell
    real(double) :: g(6), dx, sumd2phidx2, h(6), residual
    type(VECTOR) :: dir(6), locator
    type(OCTAL), pointer :: neighbourOctal
    real(double) :: q, rho, rhoe, rhou, rhov, rhow, x, qnext, pressure, flux, phi
    real(double) :: phigas, correction, xnext, px, py, pz, rm1, um1,pm1
    ...
    if (thisOctal%nDepth == nDepth) then
       ...
       thisOctal%parent%chiLine(thisOctal%parentSubcell) = 0.d0
       ...
       do subcell = 1, thisOctal%maxChildren
          ...
          if (.not.thisOctal%edgeCell(subcell)) then
             ...
             residual = fourPi*thisOctal%rho(subcell)*bigG - (sum(h(1:6)) - &
                  6.d0*thisOctal%phi_gas(subcell))/(thisOctal%subcellSize*gridDistanceScale)**2

              thisOctal%parent%chiLine(thisOctal%parentSubcell) = &
                  thisOctal%parent%chiLine(thisOctal%parentSubcell) + (thisOctal%chiline(subcell) - sumd2phidx2)

!              thisOctal%parent%chiLine(thisOctal%parentSubcell) = &
!                  thisOctal%parent%chiLine(thisOctal%parentSubcell) + (residual - sumd2phidx2)
           endif
       enddo
       thisOctal%parent%chiLine(thisOctal%parentSubcell) = &
            thisOctal%parent%chiLine(thisOctal%parentSubcell) /dble(thisOctal%maxChildren)

!       if (octalOnThread(thisOctal,1,myrankGlobal).and.(thisOctal%nDepth==maxDepthAMR)) &
!       write(*,*) "done restriction 1 ",thisoctal%parent%ndepth, thisOctal%parent%chiline(thisOctal%parentsubcell)
       ...
       do subcell = 1, thisOctal%maxChildren
          ...
          if (.not.thisOctal%edgeCell(subcell)) then
             do n = 1, nDir                
                locator = subcellCentre(thisOctal, subcell) + dir(n) * (thisOctal%subcellSize/2.d0+0.01d0*smallestCellsize)
                neighbourOctal => grid%octreeRoot
                call findSubcellLocalLevel(locator, neighbourOctal, neighbourSubcell, nDepth-1)
                call getNeighbourValues(grid, thisOctal, subcell, neighbourOctal, neighbourSubcell, dir(n), q, rho, rhoe, &
                     rhou, rhov, rhow, x, qnext, pressure, flux, phi, phigas, correction, nd, nc, xnext, px, py, pz, rm1,um1, pm1, &
                     qViscosity)
                if (nc > 0) then
                   call findSubcellTD(locator, grid%octreeRoot, neighbourOctal, neighbourSubcell)
                   call getNeighbourValues(grid, thisOctal, subcell, neighbourOctal, neighbourSubcell, dir(n), q, rho, rhoe, &
             ...
             thisOctal%chiline(subcell) = 0.!thisOctal%rho(subcell)*fourPi*bigG - sumd2phidx2 
!             if (thisOctal%nDepth == (maxDepthAMR-1))write(*,*) "done restriction 2 ",thisOctal%chiline(subcell)
       ...
       if (thisoctal%nchildren > 0) then
          do i = 1, thisoctal%nchildren, 1
             child => thisoctal%child(i)
             call restrictResiduals(child, grid, nDepth)
          end do
       endif
...
!       write(*,*) "this ",thisOctal%phi_gas(1:thisOctal%maxChildren)
  ...
  subroutine findMaxResidual(thisOctal, nDepth, residual)
    type(octal), pointer   :: thisoctal
    integer :: nDepth
    real(double) :: residual

    residual = 0.d0
    call findMaxResidualRecur(thisOctal, nDepth, residual)
  end subroutine findMaxResidual


  recursive subroutine findMaxResidualRecur(thisoctal, nDepth, residual)
    use mpi
    type(octal), pointer   :: thisoctal
    type(octal), pointer  :: child 
    ...
    if (thisOctal%nDepth == nDepth) then
       ...
       if (thisoctal%nchildren > 0) then
          do i = 1, thisoctal%nchildren, 1
             child => thisoctal%child(i)
             call findMaxResidualRecur(child, nDepth, residual)
          end do
       endif
    endif


  end subroutine findMaxResidualRecur

  recursive subroutine setCorrectionToPhiGas(thisoctal, nDepth)
    use mpi
    type(octal), pointer   :: thisoctal
    type(octal), pointer  :: child 
    integer :: i, nDepth
    ...
    if (thisOctal%nDepth == nDepth) then
       ...
       thisOctal%correction(1:thisOctal%maxChildren) = thisOctal%phi_gas(1:thisOctal%maxChildren)
    else
       if (thisoctal%nchildren > 0) then
          do i = 1, thisoctal%nchildren, 1
             child => thisoctal%child(i)
             call setCorrectionToPhiGas(child, nDepth)
          end do
       endif
    endif


  end subroutine setCorrectionToPhiGas

  recursive subroutine setCorrectionToZero(thisoctal, nDepth)
    use mpi
    type(octal), pointer   :: thisoctal
    type(octal), pointer  :: child 
    integer :: i, nDepth
    ...
    if (thisOctal%nDepth == nDepth) then
       ...
       if (thisoctal%nchildren > 0) then
          do i = 1, thisoctal%nchildren, 1
             child => thisoctal%child(i)
             call setCorrectionToZero(child, nDepth)
          end do
       endif
    endif


  end subroutine setCorrectionToZero
  ...
  recursive subroutine prolongatePhiGasold(thisoctal, ndepth)
    use mpi
    type(octal), pointer   :: thisoctal
    type(octal), pointer  :: child 
    integer :: i, ndepth
...
!             thisOctal%phi_gas(i) = thisOctal%parent%phi_gas(thisOctal%parentSubcell)
       ...
       do i = 1, thisOctal%nChildren
          child => thisOctal%child(i)
          call prolongatePhiGasold(child, nDepth)
       enddo

    endif

  end subroutine prolongatePhiGasold


  recursive subroutine prolongatePhiGasLoop(grid, thisoctal, ndepth)
    use mpi
    type(GRIDTYPE) :: grid
       ...
       do subcell = 1, thisOctal%maxChildren
          ...
          if (.not.thisOctal%edgeCell(subcell)) then
             ...
             do i = 1, nDir
                ...
                locator(i) = centre + (parent%subcellSize + 0.001d0*smallestCellSize)*dir(i)

                if (.not.inOctal(grid%octreeRoot, locator(i))) then
                   phigas(i) = parent%phi_gas(parentSubcell)
                else
                   neighbourOctal => grid%octreeRoot
...
!                      call getNeighbourValues(grid, parent, parentsubcell, neighbourOctal, &
!                           neighbourSubcell, dir(i), q, rho, rhoe, &
!                           rhou, rhov, rhow, x, qnext, pressure, flux, phi, phigas(i), correction, nd, nc, &
!                           xnext, px, py, pz, rm1,um1,&
!                           pm1, qViscosity)
!                      locator(i) = VECTOR(px, py, pz)
                      phigas(i) = neighbourOctal%phi_gas(neighbourSubcell)
                   else
                      iThread = neighbourOctal%mpiThread(neighbourSubcell)
                      loc(1) = locator(i)%x
                      loc(2) = locator(i)%y
                      loc(3) = locator(i)%z
                      call MPI_SEND(loc, 3, MPI_DOUBLE_PRECISION, iThread, tag, localWorldCommunicator, ierr)
                      call MPI_RECV(loc, 2, MPI_DOUBLE_PRECISION, iThread, tag, localWorldCommunicator, status, ierr)
                      phigas(i) = loc(1)
                   endif
             ...
             corrInterp = 0.d0
             do i = 1, ndir
                corrInterp = corrInterp + phigas(i) * weight(i)
                tot = tot + weight(i)
             enddo
             corrInterp = corrInterp / tot
             thisOctal%phi_gas(subcell) = corrInterp
       ...
       do i = 1, thisOctal%nChildren
          child => thisOctal%child(i)
          call prolongatePhiGasLoop(grid, child, nDepth)
       enddo

    endif

  end subroutine prolongatePhiGasLoop

  recursive subroutine prolongateCorrectionsLoop(grid, thisoctal, ndepth)
    use mpi
    type(GRIDTYPE) :: grid
    type(octal), pointer   :: thisoctal, parent
    type(octal), pointer  :: child , neighbourOctal
...
!                      call getNeighbourValues(grid, parent, parentsubcell, neighbourOctal, &
!                           neighbourSubcell, dir(i), q, rho, rhoe, &
!                           rhou, rhov, rhow, x, qnext, pressure, flux, phi, phigas, correction(i), nd, nc, &
!                           xnext, px, py, pz, rm1,um1,&
!                           pm1, qViscosity)
                      correction(i) = neighbourOctal%correction(neighbourSubcell)
!                      locator(i) = VECTOR(px, py, pz)
                   else
                      iThread = neighbourOctal%mpiThread(neighbourSubcell)
                      loc(1) = locator(i)%x
             ...
             corrInterp = 0.d0
             do i = 1, ndir
                corrInterp = corrInterp + correction(i) * weight(i)
                tot = tot + weight(i)
             enddo
             corrInterp = corrInterp / tot
       ...
       do i = 1, thisOctal%nChildren
          child => thisOctal%child(i)
          call prolongateCorrectionsLoop(grid, child, nDepth)
       enddo

    endif

  end subroutine prolongateCorrectionsLoop

  subroutine prolongatePhiGas(grid, nDepth)
    use mpi
    type(GRIDTYPE) :: grid
    integer :: nDepth
    integer :: iThread, j
    ...
    do iThread = 1, nHydroThreadsGlobal
       if (iThread == myrankGlobal) then
          call prolongatePhiGasLoop(grid, grid%octreeRoot, nDepth)
          loc(1) = 1.d30
          do j = 1, nHydroThreadsGlobal
             if (j /= myrankGlobal) call MPI_SEND(loc, 2, MPI_DOUBLE_PRECISION, j, tag, localWorldCommunicator, ierr) 
          enddo
       else
          call prolongationServer(grid, nDepth, iThread)
       endif
    end do
  end subroutine prolongatePhiGas

  subroutine prolongateCorrections(grid, nDepth)
    use mpi
    type(GRIDTYPE) :: grid
    integer :: nDepth
    integer :: iThread, j
    ...
    do iThread = 1, nHydroThreadsGlobal
       if (iThread == myrankGlobal) then
          call prolongateCorrectionsLoop(grid, grid%octreeRoot, nDepth)
          loc(1) = 1.d30
          do j = 1, nHydroThreadsGlobal
             if (j /= myrankGlobal) call MPI_SEND(loc, 2, MPI_DOUBLE_PRECISION, j, tag, localWorldCommunicator, ierr) 
          enddo
  ...
  subroutine prolongationServer(grid, nDepth, fromThread)
    use mpi
    type(GRIDTYPE) :: grid
    type(OCTAL), pointer :: thisOctal
    integer :: fromThread
    ...
    do
       call MPI_RECV(loc, 3, MPI_DOUBLE_PRECISION, fromThread, tag, localWorldCommunicator, status, ierr)
       if (loc(1) > 1.d29) exit
       ...
       call findSubcellLocalLevel(position, thisOctal, subcell, nDepth-1)
       loc(1) = thisOctal%phi_gas(subcell)
       loc(2) = thisOctal%correction(subcell)
       call MPI_SEND(loc, 2, MPI_DOUBLE_PRECISION, fromThread, tag, localWorldCommunicator, ierr) 
    enddo
  end subroutine prolongationServer

       


  recursive subroutine prolongatePhi(grid, thisoctal, ndepth)
    use mpi
    type(GRIDTYPE) :: grid
    type(octal), pointer   :: thisoctal, parent
    type(octal), pointer  :: child , neighbourOctal
    ...
    real(double) :: flux, phi,  correction(7), junk
    integer :: nd, nc
    ...
    if (thisoctal%ndepth == ndepth) then
       ...
       do subcell = 1, thisOctal%maxChildren
          ...
          if (.not.thisOctal%edgeCell(subcell)) then
             ...
             do i = 1, nDir
                ...
                if (.not.inOctal(grid%octreeRoot, locator(i))) then
                   correction(i) = parent%phi_gas(parentSubcell)
                else
                   neighbourOctal => grid%octreeRoot
                   ...
                   call getNeighbourValues(grid, thisOctal, subcell, neighbourOctal, neighbourSubcell, dir(i), q, rho, rhoe, &
                        rhou, rhov, rhow, x, qnext, pressure, flux, phi, correction(i), junk, nd, nc, xnext, px, py, pz, rm1,um1,&
                        pm1, qViscosity)
                   locator(i) = VECTOR(px, py, pz)
                endif
             ...
             enddo
             locator(7) = centre
             correction(7) = parent%phi_gas(parentSubcell)      
       ...
       do i = 1, thisOctal%nChildren
          child => thisOctal%child(i)
          call prolongatePhi(grid, child, nDepth)
       enddo

    endif

  end subroutine prolongatePhi
...
!       if (any(thisOctal%phi_gas > 0.d0)) write(*,*) myrankGlobal," phi ",thisoctal%phi_gas(1:4),thisOctal%ghostCell(1:4)
  ...
  recursive subroutine correctPhiGas(thisoctal, ndepth)
    use mpi
    type(octal), pointer   :: thisoctal
    type(octal), pointer  :: child 
    integer :: subcell, i, ndepth
    ...
    if (thisoctal%ndepth == ndepth) then
       ...
       do subcell = 1, thisOctal%maxChildren
          if (.not.thisOctal%edgeCell(subcell)) then
!             if (nDepth==maxDepthAMR) write(*,*) "phi ",thisOctal%phi_gas(subcell), " corr ",thisOctal%correction(subcell)
             thisoctal%phi_gas(subcell) = thisOctal%phi_gas(subcell) + thisOctal%correction(subcell)
          endif
       enddo
    else
       ...
       do i = 1, thisoctal%nchildren, 1
          child => thisoctal%child(i)
          call correctPhiGas(child, ndepth)
       end do

    endif

  end subroutine correctPhiGas

  recursive subroutine updateCorrectionWithDeltaE(thisoctal, ndepth)
    use mpi
    type(octal), pointer   :: thisoctal
    type(octal), pointer  :: child 
    integer :: subcell, i, ndepth
    ...
    if (thisoctal%ndepth == ndepth) then
       ...
       do subcell = 1, thisOctal%maxChildren
          if (.not.thisOctal%edgeCell(subcell)) then
                thisoctal%correction(subcell) =  thisOctal%correction(subcell) + thisOctal%phi_gas(subcell) 
          endif
       enddo
    else
       ...
       do i = 1, thisoctal%nchildren, 1
          child => thisoctal%child(i)
          call updateCorrectionWithDeltaE(child, ndepth)
       end do

    endif

  end subroutine updateCorrectionWithDeltaE
  ...
  recursive subroutine copyPhiGasToEtaLine(thisoctal, ndepth)
    use mpi
    type(octal), pointer   :: thisoctal
    type(octal), pointer  :: child 
    integer :: i, ndepth
    ...
    if (thisoctal%ndepth == ndepth) then
       ...
       thisOctal%etaline = thisOctal%phi_gas 
    else

       do i = 1, thisoctal%nchildren, 1
          child => thisoctal%child(i)
          call copyPhiGasToEtaLine(child, ndepth)
       end do
    endif

  end subroutine copyPhiGasToEtaLine

  recursive subroutine copyPhiGasToAdot(thisoctal, ndepth)
    use mpi
    type(octal), pointer   :: thisoctal
    type(octal), pointer  :: child 
    integer :: i, ndepth
    ...
    if (thisoctal%ndepth == ndepth) then
       ...
       do i = 1, thisoctal%nchildren, 1
          child => thisoctal%child(i)
          call copyPhiGasToAdot(child, ndepth)
       end do
    endif

  end subroutine copyPhiGasToAdot

  recursive subroutine copyAdotToPhigas(thisoctal, ndepth)
    use mpi
    type(octal), pointer   :: thisoctal
    type(octal), pointer  :: child 
    integer :: i, ndepth
    ...
    if (thisoctal%ndepth == ndepth) then
       ...
       do i = 1, thisoctal%nchildren, 1
          child => thisoctal%child(i)
          call copyAdotToPhiGas(child, ndepth)
       end do
    endif
  ...
  recursive subroutine setCorrectionsToPhiGas(thisoctal, ndepth)
    ...
    if (thisoctal%ndepth == ndepth) then
       ...
       if (.not.associated(thisOctal%correction)) allocate(thisOctal%correction(1:thisOctal%maxChildren))
       thisOctal%correction(:) = thisOctal%phi_gas(:)

    else

       do i = 1, thisoctal%nchildren, 1
          child => thisoctal%child(i)
          call setCorrectionsToPhiGas(child, ndepth)
       end do
    endif

  end subroutine setCorrectionsToPhiGas

  recursive subroutine copyEtaLineToPhigas(thisoctal, ndepth)
    use mpi
    type(octal), pointer   :: thisoctal
    type(octal), pointer  :: child 
    integer :: i, ndepth
    ...
    if (thisoctal%ndepth == ndepth) then
       ...
       thisOctal%phi_gas  = thisOctal%etaline
    else

       do i = 1, thisoctal%nchildren, 1
          child => thisoctal%child(i)
          call copyEtalinetoPhigas(child, ndepth)
       end do
    endif

  end subroutine copyEtaLineToPhigas
...
!Set up the i-1/2 flux for each cell on the grid
  recursive subroutine constructfluxCylindrical(grid, thisoctal, dt, direction, writeDebug)
    ...
    real(double) :: qViscosity(3,3), rm1, um1, pm1, correction
    integer :: nd, nc
...
!set up neighbour positions and values of the advecting quantity
  recursive subroutine setupqx(thisoctal, grid, direction)
    ...
    real(double) :: xnext, px, py, pz, qViscosity(3,3), rm1, um1, pm1, correction
    integer :: subcell, i, neighboursubcell
    integer :: nd, nc
...
!set up neighbour densities and gravtiational potentials
  recursive subroutine setuprhophi(thisoctal, grid, direction)
    ...
    type(vector) :: direction, locator, reversedirection
    integer :: nd, nc
    real(double) :: xnext, px, py, pz, rm1, um1, pm1, correction
...
!set up neighbour densities and cell interface advecting velocities - x direction
  recursive subroutine setupui(thisoctal, grid, direction, dt)
    ...
    real(double) :: rhou_i_minus_1, rho_i_minus_1, dt, correction
...
! now the cell at i+1/2
             ...
             if(rhiechow .and. .not. thisOctal%ghostcell(subcell)) then
                ...
                call findsubcelllocal(locator, neighbouroctal, neighboursubcell)
                call getneighbourvalues(grid, thisoctal, subcell, neighbouroctal, neighboursubcell, direction, q, rho, rhoe, &
                     rhou, rhov, rhow, x, qnext, pressure, flux, phi, phigas, correction, nd, nc, &
...
!note that thisOctal%flux_i is the flux at i-1/2
  recursive subroutine setupflux(thisoctal, grid, direction)
    ...
    integer :: nd,nc
    real(double) :: xnext, fac, px, py, pz, rm1, um1, pm1, correction
...
!Find the flux gradient perpendicular to the advection direction
  subroutine   normalFluxGradientSingle(f, xpos, direction, community, upper, thisOctal, subcell, neighbourOctal, &
    ...
    real(double) :: xnext, dx, px, py, pz, rm1, um1, pm1, correction
    logical :: upper
    ...
    call getneighbourvalues(grid, thisOctal, subcell, neighbouroctal, neighboursubcell, (-1.d0)*direction, q, &
         rho, rhoe, rhou, rhov, rhow, x, qnext, pressure, flux, phi, phigas, correction, nd, nc, xnext, px, py, pz, rm1,um1, pm1, &
         qViscosity)
             ...
             if(fineToCoarse) then
                ...
                if(octalOnTHread(communityOctal, communitySubcell, myRankGlobal)) then
                   call getneighbourvalues(grid, neighbouroctal, neighboursubcell, communityoctal, communitysubcell, &
                        direction, q, rho, rhoe, rhou, rhov, rhow, x, qnext, pressure, flux, phi, phigas, correction, nd, nc, &
                        xnext, px, py, pz, rm1,um1, pm1, qViscosity)
                else
                   call getneighbourvalues(grid, neighbouroctal, neighboursubcell, communityoctal, communitysubcell, &
                        community(i), q, rho, rhoe, rhou, rhov, rhow, x, qnext, pressure, flux, phi, phigas, correction, nd, nc, &
                        xnext, px, py, pz,  rm1,um1, pm1, qViscosity)
                end if
                ...
                if(octalOnTHread(communityOctal, communitySubcell, myRankGlobal)) then
                   call getneighbourvalues(grid, thisoctal, subcell, communityoctal, communitySubcell, direction, q, &
                        rho, rhoe, rhou, rhov, rhow, x, qnext, pressure, flux, phi, phigas, correction, nd, nc, xnext, px, py, pz, &
                ...
                if(octalOnthread(communityoctal, communitysubcell, myRankGlobal) .and. (nd-thisOctal%nDepth == 0)) then
                   ...
                   if(octalOnTHread(mirrorOctal, mirrorSubcell, myRankGlobal)) then
                      Call getneighbourvalues(grid, communityOctal, communitySubcell, mirrorOctal, mirrorsubcell, direction, q, &
                           rho, rhoe, rhou, rhov, rhow, x, qnext, pressure, flux, phi, phigas, correction, &
                      ...
                      Call getneighbourvalues(grid, communityOctal, communitySubcell, mirrorOctal, mirrorsubcell, community(i), &
                           q, rho, rhoe, rhou, rhov, rhow, x, qnext, pressure, flux, phi, phigas, correction, &
                           nd, nc, xnext, px, py, pz, rm1,um1, pm1, qViscosity)
                   end if
                   ...
                   if(mVec == nVec) then
                      ...
                      call getneighbourvalues(grid, thisoctal, subcell, communityoctal, communitysubcell, direction, q, &
                           rho, rhoe, rhou, rhov, rhow, x, qnext, pressure, flux, phi, phigas, correction, nd, nc, xnext, px, py, &
                           pz, rm1,um1, pm1, qViscosity)
                      ID(i) = 3
                   end if
                else
                   ID(i) = 1
...
!Probe across the mpi boundary and check if the corresponding cell is the community subcell
                   ...
                   call getneighbourvalues(grid, mirroroctal, mirrorsubcell, communityoctal, communitysubcell, direction, q, rho, & 
                        rhoe, rhou, rhov, rhow, x, qnext, pressure, flux, phi, phigas, correction, nd, nc, xnext, px, py, pz, rm1,&
                        um1, pm1, qViscosity)
                   ID(i) = 2
...
!                   if(octalOnThread(faceOctal, faceSubcell, myRankGlobal)) then
                      call getneighbourvalues(grid, mirroroctal, mirrorsubcell, faceoctal, facesubcell, direction, q, rho, &
                           rhoe, rhou, rhov, rhow, x, qnext, pressure, flux, phi, phigas, correction, nd, nc, xnext, px, py, pz, &
                           rm1,um1, pm1, qViscosity)
!                   else
...
!             call findsubcelllocal(locator, neighbouroctal, neighboursubcell)
!             call getneighbourvalues(grid, thisoctal, subcell, neighbouroctal, neighboursubcell, direction, q, rho, rhoe, &
!                  rhou, rhov, rhow, x, qnext, pressure, flux, phi, phigas, correction, nd, nc, xnext, px, py, pz, q11, q22, q33)
...
!Perform the advection on q
  ...
  subroutine AccFeedback (grid, dt)
    ...
    type(gridtype) :: grid
    real(double)   :: dt
    logical, save :: calculatedNormilization=.false.
    real(double), allocatable, save, dimension(:,:,:) ::  normConsts3
    real(double), allocatable, save, dimension(:,:,:) ::  rho3
    real(double), allocatable, save, dimension(:,:)   ::  normConsts2, vol2
    real(double), allocatable, save, dimension(:,:)   ::  rho2
    integer :: arrSize
    integer :: i,j,k
    real(double) :: r, theta,dtheta, x,y,z, mdotFw, vk
...
!    write(*,*) "starting feedback"
    if (.not. calculatedNormilization) then
       ...
       if(grid%octreeRoot%threed) then
          ...
          do i = -arrSize, arrSize
             do j = -arrSize, arrSize
                do k = -arrSize, arrSize
                   x=i-0.5
                   y=j-0.5
                   z=k-0.5
                   r=sqrt(x**2+y**2+z**2)
                   if (r>accretionRadius .and. r<(accretionRadius+AccFeedbackCells) .and. z.ne.0) then
                      theta=atan2(sqrt(x**2+y**2),z)
                      !feedback function to dive density and momentum injected back into the domain from a bipolar outflow, see cunningham et al 2011
                      normConsts3(i,j,k)=(atan(sqrt(FeedbackTheta0**2+1)*tan(theta+1.0/2/r)/FeedbackTheta0)-&
                                          atan(sqrt(FeedbackTheta0**2+1)*tan(theta-1.0/2/r)/FeedbackTheta0))/&
                                         (r**2*FeedBackTheta0*sqrt(FeedbackTheta0**2+1))  
                   end if
                end do
             end do
          end do
          normConsts3=normConsts3/sum(normConsts3)
       else if (grid%octreeRoot%twoD) then
          allocate(normConsts2(1:arrSize,1:arrSize))
          allocate(vol2(1:arrSize,1:arrSize))
          allocate(rho2(1:arrSize,1:arrSize))
          normConsts2=0.0
          vol2=0.0
          do i=1, arrSize
             do j=1, arrSize
                ...
                if (r>accretionRadius .and. r<(accretionRadius+AccFeedbackCells) .and. z.ne.0) then
                   theta=atan2(x,z)
                   dtheta=abs(atan2(x-0.5,z+0.5)-atan2(x+0.5,z-0.5))
                   normConsts2(i,j)=(atan(sqrt(FeedbackTheta0**2+1)*tan(theta+dtheta/2)/FeedbackTheta0)-&
                                     atan(sqrt(FeedbackTheta0**2+1)*tan(theta-dtheta/2)/FeedbackTheta0))/&
                                    (r**2*FeedBackTheta0*sqrt(FeedbackTheta0**2+1))
!                   write(*,*) normConsts2(i,j)
                   vol2(i,j)=((x+0.5)**2+(x-0.5)**2) *2*pi * (gridDistanceScale*smallestCellSize)**3!volume in units of smallest cells
                endif
             end do
          end do
          normConsts2=normConsts2*vol2/sum(normConsts2*vol2)/2 !proportional amount of mass that chould go into each cell (/2 as we are only considering +z here)
       else
          write(*,*) "warning: outflow feedback in 1D isnt possible"
       end if
       calculatedNormilization=.true.
    endif


    mdotFw=globalSourceArray(1)%mdot*FeedbackFw
    vk=sqrt(bigG*globalSourceArray(1)%mass/(globalSourceArray(1)%radius*gridDistanceScale))

    if (myrankGlobal.eq.1)     write(*,*) mdotFw,vk
    if (mdotFw>0 .and. globalSourceArray(1)%mass>FeedbackStartMass) then
       if(grid%octreeRoot%threed) then

       else
          rho2=normConsts2*mdotFw
          rho2=rho2*mdotFw/sum(rho2*vol2)
          call applyFeedback(grid%octreeroot, rho2, dt, vk)
       endif
   

       if(any(rho2.ne.rho2).or. any(vol2.ne.vol2))then
          if (myrankglobal<2) then
           write(*,*) normConsts2
           do i=1, arrSize
             do j=1, arrSize
                x=i-0.5
                z=j-1
                r=sqrt(x**2+z**2)
                if (r>accretionRadius .and. r<(accretionRadius+AccFeedbackCells) .and. z>1) then
                   theta=atan2(x,z)
                   dtheta=abs(atan2(x-0.5,z+0.5)-atan2(x-0.5,z+0.5))
                   write(*,*) atan(sqrt(FeedbackTheta0**2+1)*tan(theta+dtheta/2)/FeedbackTheta0)
                   write(*,*) atan(sqrt(FeedbackTheta0**2+1)*tan(theta-dtheta/2)/FeedbackTheta0)
                   write(*,*) r**2*FeedBackTheta0*sqrt(FeedbackTheta0**2+1)
                endif
             end do
           end do
           write(*,*) ";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;"
  ...
  end subroutine AccFeedback
    
  recursive subroutine applyFeedback(thisoctal, rho, dt, vk)
    use inputs_mod, only : AccFeedbackCells, FeedbackFv, accretionRadius
    type(octal), pointer :: thisOctal, child
    integer :: subcell,i,j, childi
    type(vector) :: r
    real(double) :: dt, vk
    real(double) :: rho(:,:)
    ...
    do subcell = 1, thisOctal%maxChildren
       if (thisOctal%hasChild(subcell)) then
!          if (.not.OctalOnThread(thisOctal, subcell, myRankGlobal)) cycle
          do childi = 1, thisOctal%nChildren, 1
             if (thisOctal%indexChild(childi) == subcell) then
                child => thisOctal%child(childi)
!                if (octalonthread(thisoctal, subcell, myrankGlobal))  write(*,*) subcell, childi
                call applyFeedback(child, rho, dt, vk)
                exit
             endif
          end do
       else
          if (.not.octalonthread(thisoctal, subcell, myrankGlobal) .or.&
              thisOctal%ghostcell(subcell)) cycle
          if(thisOctal%threed) then
             !not implemented
          else
             r=subcellcentre(thisOctal, subcell)-globalSourceArray(1)%position
             if (r%x > (AccFeedbackCells+accretionRadius+0.5)*smallestCellsize) cycle 
             if (abs(r%z) > (AccFeedbackCells+accretionRadius+0.5)*smallestCellsize) cycle 
             i=nint(sqrt(r%x**2+r%y**2)/smallestCellSize+0.50001)
             j=nint(abs(r%z/smallestCellSize+0.5))
             if (sqrt(real(i**2+j**2))<(AccFeedbackCells+accretionRadius+0.5).and.&
                 sqrt(real(i**2+j**2))>AccFeedbackCells) then
!                write(*,*) "injecting feedback at", i,j
                call normalize(r)
                thisOctal%rho(subcell)=thisOctal%rho(subcell)+dt*rho(i,j)                
                thisOctal%rhou(subcell)=thisOctal%rhou(subcell)+dt*rho(i,j)*feedbackFv*vk * r%x
                thisOctal%rhow(subcell)=thisOctal%rhow(subcell)+dt*rho(i,j)*feedbackFv*vk * r%z
             endif
          endif
       endif
    enddo
...
!Perform a single hydrodynamics step, in the x direction, for the 1D case. 
  subroutine  hydrostep1d(grid, dt, npairs, thread1, thread2, nbound, group, ngroup)
    use inputs_mod, only: imposeFixing
    type(gridtype) :: grid
    real(double) :: dt
    type(vector) :: direction
...
!Boundary conditions
    if (imposefixing) then 
       call fixOctalValues(grid) 
    end if
...
!   if (writeoutput) write(*,*) "Total mass on grid after accretion  is ", totalmass/mSol
   ...
   if ((globalnSource > 0).and.nBodyPhysics.and.moveSources) then
      ...
      call updateSourcePositions(globalsourceArray, globalnSource, timestep, grid)
      if (writeoutput) write(*,'(a, 3(es9.2,1x))') "Source 1 pos: ", globalsourceArray(1)%position 
   else
      globalSourceArray(1:globalnSource)%velocity = VECTOR(0.d0,0.d0,0.d0)
   endif
...
!Perform a single hydrodynamics step, in r and z directions, for the 2D cylindrical case.     
  subroutine hydroStep2dCylindrical(grid, timeStep, nPairs, thread1, thread2, nBound, group, nGroup)
    use inputs_mod, only : doselfGrav, doGasGravity, alphaViscosity, imposeFixing
    type(GRIDTYPE) :: grid
    logical :: selfGravity
    integer :: nPairs, thread1(:), thread2(:), nBound(:)
    integer :: group(:), nGroup
    ...
    do iDir = 1, 3
       ...
       if (AccretionFeedback) call AccFeedback(grid, dt)
       if (hydroSpeedLimit /= 0.) call limitSpeed(grid%octreeRoot)
...
!Perform a single hydrodynamics step, in r and z directions, for the 2D cylindrical case.     
  subroutine hydroStep2dCylindrical_amr(grid, timeStep, nPairs, thread1, thread2, nBound, group, nGroup)
    ...
    call writeInfo("Labeling boundaries", TRIVIAL)
    call checkForBoundaries2Dcylindrical(grid%octreeRoot, grid)
...
!             if(speed > hydroSpeedLimit) speed = hydrospeedlimit
             ...
             speed = max(speed, soundSpeed(thisOctal, subcell))
...
! NB confusion regarding 2d being x,z rather than x,y
                   ...
                   if (thisOctal%twod.or.thisOctal%oneD) then
                     ...
                     if (abs(dir%x) > 0.9d0) then
                         thisOctal%tempStorage(subcell,3) = 0.0
                         thisOctal%tempStorage(subcell,4) = bOctal%rhov(bSubcell)
                         thisOctal%tempStorage(subcell,5) = bOctal%rhow(bSubcell)
                         thisOctal%tempStorage(subcell,8) = bOctal%phi_i(bsubcell)
                      endif
                      if (abs(dir%z) > 0.9d0) then
                         thisOctal%tempStorage(subcell,3) = bOctal%rhou(bSubcell)
                         thisOctal%tempStorage(subcell,4) = bOctal%rhov(bSubcell)
                         thisOctal%tempStorage(subcell,5) = 0.0
                         thisOctal%tempStorage(subcell,8) = bOctal%phi_i(bsubcell)
                      endif
!                      if ((abs(dir%x) > 0.5d0).and.abs(dir%z) > 0.5d0) then
!                         thisOctal%tempStorage(subcell,3) = -bOctal%rhou(bSubcell)
...
!                      endif
                      ...
                      if (abs(dir%y) > 0.9d0) then
                         thisOctal%tempStorage(subcell,3) = bOctal%rhou(bSubcell)
                         thisOctal%tempStorage(subcell,4) = 0.0
                         thisOctal%tempStorage(subcell,5) = bOctal%rhow(bSubcell)
                         thisOctal%tempStorage(subcell,8) = bOctal%phi_i(bsubcell)
                      endif
                      if (abs(dir%z) > 0.9d0) then
...
!                  rhout, rhovt, rhowt, x, qnext, pressure, flux, phi, phigas, correction, correction, nd, nc, xnext, px, py, pz,  rm1,um1, pm1, qViscosity)
...
!calculate the new gradient
  ...
  recursive subroutine gaussSeidelSweep(thisOctal, grid, fracChange,nDepth, onlyCellsWithChildren)
    ...
    integer :: n, ndir
    real(double) ::  g(6), dx
    real(double) :: fracChange, sumd2phidx2, correction
    integer :: nd, nc
  ...
  recursive subroutine calculateResiduals(thisOctal, grid, nDepth)
    ...
    integer :: n, ndir
    real(double) ::  g(6), dx
    real(double) :: sumd2phidx2, correction
    integer :: nd, nc
    real(double), parameter :: maxM = 100000.d0
    real(double) :: xnext, px, py, pz, rm1, um1, pm1
    real(double), parameter :: SOR = 1.2d0
  ...
  recursive subroutine calculateResidualsFineCoarse(thisOctal, grid, nDepth)
    use mpi
    type(GRIDTYPE) :: grid
    integer :: nDepth
    type(octal), pointer   :: thisOctal
    ...
    real(double) :: xnext, px, py, pz, rm1, um1, pm1
    real(double), parameter :: SOR = 1.2d0
    real(double) :: x1, x2, thisR, dxArray(6), phiInterface(6), dfdrbyr, d2phidx2(6),g2(6)
    nDir = 6
    if (thisOctal%twoD) then
       nDir = 4
       probe(1) = VECTOR(1.d0, 0.d0, 0.d0)
    ...
    if (thisOctal%nDepth == nDepth) then
       ...
       do subcell = 1, thisOctal%maxChildren
          ...
          if (.not.thisOctal%edgeCell(subcell)) then
             ...
             do n = 1, nDir
                ...
                dxArray(n) = dx
                g(n) =   (correction - thisOctal%correction(subcell))/dxArray(n)
             enddo

             !get the gravitational potential values at the cell interface
             do n = 1, nDir
                dx = sign(thisOctal%subcellSize/2.d0,dxArray(n))
                phiInterface(n) = thisOctal%correction(subcell) +  g(n)*(dx*gridDistanceScale)
             end do
             ...
             do n = 1, nDir
                dx = sign(thisOctal%subcellSize/2.d0,dxArray(n))
                g2(n) = (phiInterface(n) - thisOctal%correction(subcell))/(dx*gridDistanceScale)
             end do
       ...
       if (thisOctal%nChildren > 0) then
          do i = 1, thisOctal%nChildren, 1
             child => thisOctal%child(i)
             call calculateResidualsFineCoarse(child, grid, ndepth)
          end do
       endif
    endif
  end subroutine calculateResidualsFineCoarse
...
!calculate the new gradient
  ...
  recursive subroutine calculateResiduals2(thisOctal, grid, nDepth)
    ...
    integer :: nDepth
    real(double) :: x1, x2
    real(double) ::  g(6), dx, dxArray(6), g2(6), phiInterface(6),ptemp(6), correction
...
!             if ((myrankglobal == 5).and.(ndepth==4)) write(*,*) "correction: ",g(1:6)
             ...
             oldPhi = thisOctal%correction(subcell)
             newerPhi = thisOctal%correction(subcell) + deltaT*(sumd2phidx2 - thisOctal%source(subcell))


             newPhi = (1.d0-SOR)*oldPhi + SOR*newerPhi

             if (thisOctal%source(subcell) /= 0.d0) &
...
!             thisOctal%etacont(subcell) = newPhi
  ...
  recursive subroutine gaussSeidelFineCoarse(thisOctal, grid, fracChange, ndepth, onlyCellsWithChildren, black, red)
    use inputs_mod, only : smallestCellSize
    use mpi
    type(GRIDTYPE) :: grid
    logical, optional :: black, red
...
!calculate the new gradient
       ...
       do i = 1, thisOctal%nChildren, 1
          child => thisOctal%child(i)
          call gaussSeidelFineCoarse(child, grid, fracChange, ndepth, onlyCellsWithChildren)
       end do
    endif


  end subroutine gaussSeidelFineCoarse
  ...
  recursive subroutine gaussSeidelSweepForDeltaE(thisOctal, grid, ndepth, black, red)
    ...
    integer :: nDepth
    real(double) ::  g(6)
    real(double) :: deltaT, newPhi, newerPhi, sumd2phidx2
    integer :: nd
    real(double), parameter :: maxM = 100000.d0
    real(double) :: xnext, oldphi, px, py, pz, rm1, um1, pm1, thisR, correction
    real(double), parameter :: SOR = 1.d0
...
!           thisOctal%etacont(subcell) = newPhi
  ...
  end subroutine gaussSeidelSweepForDeltaE

  recursive subroutine gaussSeidelSweepForDeltaEfinecoarse(thisOctal, grid, ndepth, black, red)
    use inputs_mod, only : smallestCellSize
    use mpi
    type(GRIDTYPE) :: grid
    logical, optional :: black, red
    ...
    real(double) :: xnext, oldphi, px, py, pz, rm1, um1, pm1, thisR, correction
    real(double), parameter :: SOR = 1.d0
    real(double) :: dx,x1,x2,dxArray(6), phiInterface(6), dfdrbyr, d2phidx2(6), g2(6)
    integer :: nc
...
!calculate the new gradient
       ...
       do i = 1, thisOctal%nChildren, 1
          child => thisOctal%child(i)
          call gaussSeidelSweepforDeltaEfinecoarse(child, grid, ndepth, black, red)
       end do
    endif


  end subroutine gaussSeidelSweepForDeltaEfinecoarse

  recursive subroutine modifyResidual(thisOctal, grid, ndepth)
    use inputs_mod, only : smallestCellSize
    use mpi
    type(GRIDTYPE) :: grid
    type(octal), pointer   :: thisOctal
    ...
    integer :: nd
    real(double), parameter :: maxM = 100000.d0
    real(double) :: xnext, px, py, pz, rm1, um1, pm1, thisR, correction
    real(double), parameter :: SOR = 1.2d0
    integer :: nc
    ...
    if ((thisOctal%nDepth == nDepth)) then
       ...
       do subcell = 1, thisOctal%maxChildren
          ...
          if (.not.thisOctal%edgeCell(subcell)) then
             ...
             thisOctal%chiLine(subcell) = thisOctal%chiline(subcell) - sumd2phidx2
       ...
       do i = 1, thisOctal%nChildren, 1
          child => thisOctal%child(i)
          call modifyResidual(child, grid, ndepth)
       end do
    endif


  end subroutine modifyResidual

  recursive subroutine modifyResidualFineCoarse(thisOctal, grid, ndepth)
    use inputs_mod, only : smallestCellSize
    use mpi
    type(GRIDTYPE) :: grid
    type(octal), pointer   :: thisOctal
    ...
    real(double), parameter :: SOR = 1.2d0
    integer :: nc
    real(double) :: x1, x2, dx, phiInterface(6), dfdrbyr, g2(6), d2phidx2(6),dxArray(6)
...
!calculate the new gradient
       ...
       do i = 1, thisOctal%nChildren, 1
          child => thisOctal%child(i)
          call modifyResidualFineCoarse(child, grid, ndepth)
       end do
    endif


  end subroutine modifyResidualFineCoarse
...
!             thisOctal%chiline(subcell) = frac
  ...
  recursive subroutine gSweepLevel(thisOctal, grid, deltaT, fracChange, fracChange2, ghostFracChange, nDepth)
    ...
    real(double) :: sorFactor, deltaPhi, dx, sumd2phidx2, fracChange2, correction
    real(double) :: xnext, px, py, pz,qViscosity(3,3), rm1, um1, pm1
    sorFactor = 1.2d0
...
!                frac = abs((sumd2phidx2 - fourPi*gGrav*thisOctal%rho(subcell))/(fourPi*gGrav*thisOctal%rho(subcell)))
  ...
  subroutine multiGridVcycle(grid, nPairs, thread1, thread2, nBound, group, nGroup, fromDepth)
    ...
    call calculateResidualsFineCoarse(grid%octreeRoot, grid, maxDepth)
...
!             call gaussSeidelSweep2(grid%octreeRoot, grid, fracChange, iDepth, onlyCellsWithChildren=.false.)
             call gaussSeidelfinecoarse(grid%octreeRoot, grid, fracChange, iDepth, onlyCellsWithChildren=.false.)
             call exchangeAcrossMPIboundaryLevel(grid, nPairs, thread1, thread2, nBound, group, nGroup, iDepth)
...
!       call writeInfo("Beginning up part of V-cycle", TRIVIAL)
 ...
 !         call exchangeAcrossMPIboundaryLevel(grid, nPairs, thread1, thread2, nBound, group, nGroup, iDepth)
 !         call exchangeAcrossMPIboundaryLevel(grid, nPairs, thread1, thread2, nBound, group, nGroup, iDepth-1)
          call prolongateCorrections(grid, iDepth) ! from depth above

!          if (idepth==(mindepth+1)) call setrhou(grid%octreeRoot, idepth)

          call exchangeAcrossMPIboundaryLevel(grid, nPairs, thread1, thread2, nBound, group, nGroup, iDepth)
          call modifyResidualFineCoarse(grid%octreeRoot, grid, iDepth)
...
!             call gaussSeidelSweepForDeltaE(grid%octreeRoot, grid, iDepth )!,black = .true.)
             ...
             call gaussSeidelSweepForDeltaEfinecoarse(grid%octreeRoot, grid, iDepth )!,black = .true.)
...
!          thisOctal%phi_i(subcell) = - bigG*sourcemass(1)/R
  ...
  subroutine selfGrav(grid, nPairs, thread1, thread2, nBound, group, nGroup, multigrid, onlyChanged, doMultigridVcycle)
    use inputs_mod, only :  maxDepthAMR, dirichlet, simpleGrav, gravtol, forceVcycle
    use mpi
    type(gridtype) :: grid
    logical, optional :: multigrid, onlyChanged, doMultigridVcycle
    logical :: doOnlyChanged
    ...
    if (present(doMultigridVcycle).or.forceVcycle) then
       ...
       if ((maxDepthAMR /= minDepthAMR).and.(amr3d)) then
          ...
          call multiGridVcycle(grid, nPairs, thread1, thread2, nBound, group, nGroup) !, fromdepth=minDepthAMR) xxxxxxxxxxxxx
          goto 666
       endif
...
!          call exchangeAcrossMPIboundaryLevel(grid, nPairs, thread1, thread2, nBound, group, nGroup, i-1)
!          call exchangeAcrossMPIboundaryLevel(grid, nPairs, thread1, thread2, nBound, group, nGroup, i)
!          call prolongatePhiGas(grid,i)
!       enddo
...
!
  ...
  subroutine doMyAccretion(grid, sourceArray, nSource, timeStep)
    ...
    if (accretionFeedback) then
       accretedMass = accretedMass(1) * (1-feedbackFw)
       
       accretedLinMomentum(1)%x = accretedLinMomentum(1)%x * (1-feedbackFw)
       accretedLinMomentum(1)%y = accretedLinMomentum(1)%y * (1-feedbackFw)
       accretedLinMomentum(1)%z = accretedLinMomentum(1)%z * (1-feedbackFw)

       accretedAngMomentum(1)%x = accretedAngMomentum(1)%x * (1-feedbackFw)
       accretedAngMomentum(1)%y = accretedAngMomentum(1)%y * (1-feedbackFw)
       accretedAngMomentum(1)%z = accretedAngMomentum(1)%z * (1-feedbackFw)
    endif
...
recursive subroutine checkSetsAreTheSame(thisOctal)
  ...
  recursive subroutine performGasAccretion(thisOctal, accretedMass, accretedLinMomentum, accretedAngMomentum, &
       source, nSource)
    use mpi
    use inputs_mod, only : rhoThreshold, geometry, smallestCellSize, gridDistanceScale
    type(SOURCETYPE) :: source(:)
       ...
       if (thisoctal%haschild(subcell)) then
          ...
          if (iSource > 0) then
             ...
             if (thisOctal%threeD.or.spherical) then
                ...
                r = cellCentre%x*gridDistanceScale
                rv = thisOctal%rhov(subcell) / (r * thisOctal%rho(subcell))
                cellVelocity = VECTOR(thisOctal%rhou(subcell) / thisOctal%rho(subcell), &
                     rv, &
                     thisOctal%rhow(subcell) / thisOctal%rho(subcell))
...
!                   write(*,*) myrankGlobal, " localaccretedmass ",localaccretedmass
                   ...
                   if (thisOctal%threed.or.spherical) then
                      ...
                      write(*,*) "gasmom ", gasmom
                      write(*,*) "lmom ", localaccretedMom
                      gasMom = gasMom - localaccretedMom
                      thisOctal%rho(subcell) = rhoThreshold
                      cellMass = thisOctal%rho(subcell) * cellVolume(thisOctal, subcell) * 1.d30
                      cellVelocity = gasMom / cellMass
                      write(*,*) "vel ", cellvelocity
                      thisOctal%rhou(subcell) =  thisOctal%rho(subcell) * cellVelocity%x
                      thisOctal%rhov(subcell) =  thisOctal%rho(subcell) * cellVelocity%y
                      thisOctal%rhow(subcell) =  thisOctal%rho(subcell) * cellVelocity%z
...
!           write(*,*) "adding stellar wind at rate ",(mdot/msol)*365.25d0*24.d0*3600.d0
          ...
          if (r < smallestCellSize*stellarWindRadius) then
             initialVelocity = sqrt( thisoctal%rhou(subcell)**2 + thisOctal%rhov(subcell)**2 + &
                thisOctal%rhow(subcell)**2 )/thisoctal%rho(subcell)/1.d5 
             ...
             currentVelocity = sqrt( thisoctal%rhou(subcell)**2 + thisOctal%rhov(subcell)**2 + &
                thisOctal%rhow(subcell)**2 ) /thisoctal%rho(subcell)/1.d5
             if (((currentVelocity - initialVelocity) > 1.d2) .or. (currentvelocity > 500.d0)) then
                  write(*,*) "velocity increase caused by stellarwind, mstar:", source%mass/msol
                  write(*,'(e9.2,e9.2,e9.2,a5)') cellcentre%x, cellcentre%y, cellcentre%z, " pos"
                  write(*,'(e8.1,e8.1,e8.1,a16)') initialVelocity, currentVelocity, &
                   currentVelocity/initialVelocity, " old, new, ratio"
             endif

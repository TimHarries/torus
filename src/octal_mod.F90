...
!    TYPE(vector), DIMENSION(8)         :: linearvelocity       ! DAR very temporary velocity expires 28 Feb 2009
    ...
    logical, DIMENSION(:), pointer                       :: blackBody  => null()   ! cell emits as blackbody
...
! time dependent RT stuff
    ...
    real(double), pointer :: kromeIntensity(:,:) => null() ! krome intensities for photochemistry
...
!    real(double), pointer :: CII_CCOEFFS(:)
    ...
    type(VECTOR), pointer :: sourceRadMom(:) => null()
    type(VECTOR), pointer :: diffuseRadMom(:) => null()
    type(VECTOR), pointer :: dustRadMom(:) => null()
    type(VECTOR), pointer :: gasRadMom(:) => null()
    type(VECTOR), pointer :: kappaTimesFlux(:) => null()
    type(VECTOR), pointer :: sourceKappaTimesFlux(:) => null()
    type(VECTOR), pointer :: diffuseKappaTimesFlux(:) => null()
    type(VECTOR), pointer :: dustKappaTimesFlux(:) => null()
    type(VECTOR), pointer :: gasKappaTimesFlux(:) => null()
...
!          endif
       ...
       if(cart2d) then
!          v = (thisOctal%subcellSize**2)*8.8e9
          v = (thisOctal%subcellSize**3)
       else
          rVec = subcellCentre(thisOctal,subcell)

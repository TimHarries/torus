...
!  real :: meanr_line = 0., meanr_cont = 0.
  ...
  type(VECTOR) :: xAxisImage, yAxisImage
  logical :: ok
...
!  logical :: rankComplete
  ...
  real(double) :: weightsource, positionAngle
...
! From here we do multiple runs if required !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  phaseLoop: do iPhase = nStartPhase, nEndPhase
     ...
     yAxisImage = VECTOR(0.d0, 1.d0, 0.d0)
...
!                       xsample(nsample), abs(xsample(nsample)-xsample(nsample-1))/xsample(nsample)
                  if (sigma/mean < 0.001d0) converged = .true.
               endif
           enddo
...
!           call writeImage(obsImageSet(i1), specfile, objectDistance, imageInArcsec, lambda_eff, bandwidth)
           ...
           if (.not.PRESENT(returnImage)) then
              ...
              if (polarizationImages) then
                 ...
                 write(specFile,'(a,a)') trim(header)//"_qphir2.fits"
                 call writeFitsImage(obsImageSet(i1), trim(specfile), objectDistance, "qphir2", &
                      getFluxUnits(imNum),  getAxisUnits(imNum), real(lambda_eff))
...
!$OMP SHARED(ramanSourceVelocity, doRaman, xAxisImage, yAxisImage) &

...
!       endif
  ...
  function iras04158Disc(point, grid) result (rhoOut)
    use input_variables
    TYPE(gridtype), INTENT(IN) :: grid
    TYPE(octalVector), INTENT(IN) :: point
    real(double) :: r, h, rhoOut, warpHeight, fac
    real(double) :: kspiral
    integer :: nspiral1
    real(double) :: phase(10), test1, test2
    real(double), save ::someval, someval2
    integer :: i
    real(double) :: phi
...
!       endif
    ...
    rhoOut = max(rhoOut, 1.d-30)
!    write(*,*) "POINT",r,point%z
!    write(*,*) "DENSITY",rho0,rhoout/(2.*mhydrogen)
!    write(*,*) "h", h
!    write(*,*) "alphabit",(dble(rInner/r))**dble(alphaDisc),"rinner", rinner
!    write(*,*) "fac",exp(fac)
    out = 'nh2'
  !  test1 = dble(rho0)*(exp(fac)*(dble(rInner/r))**alphaDisc)/(2.*mhydrogen)
  !  test2 = dummyreadparameterfrom2dmap(point,out,.true.)
  !  someval = someval + test1/test2
  !  someval2 = someval2 + (test1/test2)**2
!    write(*,*) "nh2", test1, test2, test1/ test2

 !   write(*,*) "someval", someval,someval2


  end function iras04158Disc

 function dummyreadparameterfrom2dmap(point,output,dologint,grid) result(out)
   ...
   real(double) :: out
   character(len=10) :: output,in

   logical, optional :: dologint
   logical :: logint

   real(double) :: dave, christophe
   ...
   !!! TEST ROUTINE!!!
      ...
      do ri = 1, nr
         ...
         do zi = 1,nz
            ...
            christophe = nh2(ri,zi)

            dave = iras04158Disc(rvec,grid) / (2.* mhydrogen)
!            write(*,*) rvec
            write(*,*) christophe, dave, christophe / dave

         enddo
      enddo
      do ri = 1, nr

         r = (rgrid(ri)+rgrid(max(ri+1,nr)))/2.
         ...
         do zi = 1,nz
            ...
            rvec%z = (zgrid(ri,zi)+zgrid(ri,max(zi+1,nz)))/2.
            in ='nh2'
            christophe = parameterfrom2dmap(rvec,in,.true.)
...
!        d = sqrt((rmax-rmin)**2+(z2min-z1min)**2)
     ...
     if (abs(u) .gt. 1. .or. abs(v) .gt. 1) stop
     if ((r-rmin)*(r-rmax) .gt. 0. .and. (r1 .ne. r2)) stop
     if ((z-z1min)*(z-z1max) .gt. 0. .and. (z11 .ne. z12)) stop
     if ((z-z2min)*(z-z2max) .gt. 0. .and. (z21 .ne. z22)) stop

     if(output .eq. 'abundance') then
        
        out = (1-v) * ((1-u) * abundance(r1,z11) + u * abundance(r2,z21)) +  &
                  v * ((1-u) * abundance(r1,z21) + u * abundance(r2,z22))
   ...
   end function dummyreadparameterfrom2dmap 

   function parameterfrom2dmap(point,output,dologint,grid) result(out)
...
!        d = sqrt((rmax-rmin)**2+(z2min-z1min)**2)
   ...
   end function parameterfrom2dmap

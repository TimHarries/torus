...
! write(*,*) my_rank,"random seed",iseed(1:isize)
  ...
  if (my_rank/=0) writeoutput = .false.
...
!! Note: The following is commented out since in the new method each node
!! makes identical grid (this is achieve by passing a same a seed for
!! the random number generator to all nodes.  (RK)
!!
...
!           call fill_in_empty_octals(young_cluster,grid%octreeRoot,sphData)
        ...
        case DEFAULT
           call initFirstOctal(grid,amrGridCentre,amrGridSize, amr2d, &
           ...
           if (mie) then
              do i = 1, nDustType
                 ...
                 call plot_AMR_values(grid, message, "x-z", 0., &
                      trim(message)//".ps/vcps", .false., .false., &
                      nmarker, xmarker, ymarker, zmarker, width_3rd_dim, show_value_3rd_dim, boxfac=zoomfactor)
              enddo
           endif
        end if
...
!                (twoPi*((rOuter*1.e10)**(betaDisc-alphaDisc+2) - (rInner*1.e10)**(betaDisc-alphaDisc+2))*1.*auToCm)
   ...
   call MPI_BARRIER(MPI_COMM_WORLD, ierr)
     ...
     if (photoIonization) then
        ...
        call photoIonizationLoop(grid, source, nSource, nLambda, lamArray, readlucy, writelucy, &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     if ((geometry == "ttauri" .or. geometry == "romanova") &
          .and. formalsol) then
  if (my_rank==1) &
        call tune(6, "One Formal Sol") ! start a stopwatch  
        write(*,*) " "
	...
	! As of 04-aug-2006, this routine does not include the
        ...
        write(*,*) " "
  if (my_rank==1) &
        call tune(6, "One Formal Sol") ! stop a stopwatch
     ...
     if (grid%geometry == "donati") then
        ...
        do i = 1, 100000
           call initPhoton(thisPhoton, grid, nLambda, xArray, sourceSpectrum, &
                ...
                curtains, starSurface, forcedWavelength, usePhotonWavelength, VoigtProf, outVec)
           ...
           select case(grid%geometry)
              ...
              case DEFAULT
                 call initPhoton(thisPhoton, grid, nLambda, xArray, sourceSpectrum, &
                      ...
                      nSpot, chanceSpot, thetaSpot, phiSpot, fSpot, spotPhoton,  probDust, weightDust, weightPhoto,&
                      narrowBandImage, vMin, vMax, source, nSource, rHatinStar, energyPerPhoton, filters, mie,&
                      curtains, starSurface, forcedWavelength, usePhotonWavelength, VoigtProf, outVec, dopShift=dopShift)

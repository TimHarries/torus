...
! finished creating equation 10, now solve it to find new level populations
 ...
 if(.not.isnan(matrixB(1))) then
     ...
     Write(*,*) temperature,npops(1),npops(2),npops(3),npops(4)     
   endif
...
!if(isnan(matrixB(1)) .and. .false.) then
!   write(*,*) "MatrixA"
!  do i = 1,nlevels+1
!     write(*,'(9(f9.6,x))') matrixA(i,1:nlevels+1)
!  enddo
!   write(*,*) "MatrixE"
!  do i = 1,nlevels+1
!     write(*,'(9(f9.6,x))') matrixE(i,1:nlevels+1)
!  enddo
!   write(*,*) "MatrixB"
!  do i = 1,nlevels+1
!     write(*,*) matrixB(i)
...
!   write(*,*) "MatrixC"
!  do i = 1,nlevels+1
!     write(*,'(9(f9.6,x))') matrixC(i,1:nlevels+1)
!  enddo
!   write(*,*) "MatrixD"
!  do i = 1,nlevels+1
!     write(*,*) MatrixD(i)
...
!  write(*,*) "collmatrix"
!  do i = 1,nlevels+1
!     write(*,'(9(f9.6,x))') collmatrix(i,1:nlevels+1)
!  enddo

!call luSlv(matrixA(1:nlevels,1:nlevels), matrixB(1:nlevels), nLevels)

!   write(*,*) "MatrixA"
!  do i = 1,nlevels
!     write(*,'(9(f9.6,x))') matrixA(i,1:nlevels)
!  enddo
!   write(*,*) "MatrixE"
!  do i = 1,nlevels
!     write(*,'(9(f9.6,x))') matrixE(i,1:nlevels)
!  enddo
!   write(*,*) "MatrixB"
!  do i = 1,nlevels
!     write(*,*) matrixB(i)
...
!   write(*,*) "MatrixC"
!  do i = 1,nlevels
!     write(*,'(9(f9.6,x))') matrixC(i,1:nlevels)
!  enddo
!   write(*,*) "MatrixD"
!  do i = 1,nlevels
!     write(*,*) MatrixD(i)
...
!  write(*,*) "collmatrix"
!  do i = 1,nlevels
!     write(*,'(9(f9.6,x))') collmatrix(i,1:nlevels)
!  enddo

!  write(*,*) "determ A",determ(matrixA,nlevels+1)
!  write(*,*) "determ Asmall",determ(matrixA(1:nlevels,1:nlevels),nlevels)

!  write(*,*) "determ c",determ(matrixC,nlevels+1)
!  write(*,*) "determ csmall",determ(matrixC(1:nlevels,1:nlevels),nlevels)

!  write(*,*) "determ e",determ(matrixE,nlevels+1)
!  write(*,*) "determ esmall",determ(matrixE(1:nlevels,1:nlevels),nlevels)

	!write(*,*) mval, DETERM(matrixA,nlevels+1)
...
subroutine LTEpops(thisMolecule, temperature, levelpops)
  ...
  do i=2,thisMolecule%nlevels
     levelpops(i) = levelpops(i-1) * thisMolecule%g(i)/thisMolecule%g(i-1) * &
      exp(-abs(thisMolecule%energy(i)-thisMolecule%energy(i-1)) / (kev*temperature))
...
! this subroutine calculates the maximum fractional change in the first 6 energy levels
  recursive subroutine  swapPops(thisOctal, maxFracChangePerLevel, avgFracChange, counter, &
                         convergencearray, iter, nVoxels,itransdone,fixedrays)
    ...
    do subcell = 1, thisOctal%maxChildren
       if (thisOctal%hasChild(subcell)) then
          ...
          do i = 1, thisOctal%nChildren
             if (thisOctal%indexChild(i) == subcell) then
                child => thisOctal%child(i)
                call swapPops(child, maxFracChangePerLevel, avgFracChange, counter, convergenceArray, iter, &
         ...
         newFracChangePerLevel = abs(((thisOctal%newMolecularLevel(subcell,1:ntrans) - &
          ...
          enddo
             ...
             if(maxval(temp, mask = (.not. itransdone(1:6))) < 5. * tolerance) then
                counter(3,ntrans + 1) = counter(3,ntrans + 1) + 1
                if(maxval(temp, mask = (.not. itransdone(1:6))) < 2. * tolerance) then
                   counter(2,ntrans + 1) = counter(2,ntrans + 1) + 1
                   if(maxval(temp, mask = (.not. itransdone(1:6))) < tolerance) then
...
!          if(fixedrays .and. modulus(SubcellCentre(thisOctal,subcell)) .lt. 1e7 .and. modulus(SubcellCentre(thisOctal,subcell)) .gt. 3e6) then
          if(.not. fixedrays) then
             if (maxval(temp, mask = (.not. itransdone(1:6))) .gt. tolerance * 6.) then
                thisOctal%molecularLevel(subcell,:) = &
                     thisOctal%newmolecularLevel(subcell,:) - (0.2)*diff! (enhanced) update molecular levels
             else
                thisOctal%molecularLevel(subcell,:) = &
                     thisOctal%newmolecularLevel(subcell,:) - (0.3)*diff! (damped) update molecular levels - reduced from 0.2 to quicken convrgnce
             endif
             
             if(maxval(temp, mask = (.not. itransdone(1:6))) .lt. tolerance * 2.)then
                thisOctal%molecularLevel(subcell,:) = &
                     thisOctal%newmolecularLevel(subcell,:) - 0.5 * diff! (damped) update molecular levels - reduced from 0.2 to quicken convrgnce
             endif
             ...
             if (maxval(temp, mask = (.not. itransdone(1:6))) .gt. tolerance * 10.) then
                thisOctal%molecularLevel(subcell,:) = &
                     thisOctal%newmolecularLevel(subcell,:) - (0.00)*diff! (enhanced) update molecular levels
             else
                thisOctal%molecularLevel(subcell,:) = &
                     thisOctal%newmolecularLevel(subcell,:) - (0.0)*diff! (damped) update molecular levels - reduced from 0.2 to quicken convrgnce
             endif
             
             if(maxval(temp, mask = (.not. itransdone(1:6))) .lt. tolerance * 5.)then
                thisOctal%molecularLevel(subcell,:) = &
          ...
          convergencearray(iter,l,:) = thisOctal%molecularLevel(subcell,:)
 
       endif
    enddo
    deallocate(newFracChangePerLevel)
    ...
    do subcell = 1, thisOctal%maxChildren
       if (thisOctal%hasChild(subcell)) then
          ...
          do i = 1, thisOctal%nChildren, 1
             if (thisOctal%indexChild(i) == subcell) then
                child => thisOctal%child(i)
                call allocateMolecularLevels(grid, child, thisMolecule, restart)
                exit
             end if
          end do
       else
...
!             thisOctal%molecularLevel = 1.d-20
             ...
             do iTrans = 1, thisMolecule%nTrans
                if(itrans .eq. 1) then
                   thisOctal%molecularLevel(subcell,iTrans) = 1.
                else
                   thisOctal%molecularLevel(subcell,iTrans) = thisOctal%molecularLevel(subcell,iTrans-1) * &
                                                              thisMolecule%g(iTrans)/thisMolecule%g(iTrans-1) * &
                                                              exp(-abs(thisMolecule%energy(itrans)-thisMolecule%energy(itrans-1)) &
                                                              / (kev*thisOctal%temperature(subcell)))
                endif
                molsum = molsum + thisOctal%molecularLevel(subcell,iTrans) 
             enddo

             thisOctal%molecularLevel(subcell,:) =  thisOctal%molecularLevel(subcell,:) / molsum
!             write(*,'(5(f7.5,2x))') thisOctal%molecularLevel(subcell,1:5)
...
!                else
                   !thisOctal%jnu(subcell,i) = 1.d-20
                   thisOctal%jnu(subcell,i) = 10. * Bnu(thisMolecule%transFreq(i), tcbr)
!                endif
             enddo
          endif
    ...
    do subcell = 1, thisOctal%maxChildren
       if (thisOctal%hasChild(subcell)) then
          ...
          do i = 1, thisOctal%nChildren, 1
             if (thisOctal%indexChild(i) == subcell) then
                child => thisOctal%child(i)
                call SafeAllocateMolecularLevels(grid, child, thisMolecule, restart)
                exit
             end if
          end do
       else
          ...
          if(dorestart) then
             ...
             allocate(thisOctal%newmolecularLevel(1:thisOctal%maxChildren, 1:thisMolecule%nLevels))
             thisOctal%newmolecularLevel = 1.d-5
...
!             endif
             ...
             thisOctal%molecularLevel = 1.d-5
    ...
    enddo
  end subroutine SafeallocateMolecularLevels
...
pure function phiProf(dv, b) result (phi)
    real(double) , intent(in):: dv, b
    real(double) :: fac, phi

    fac = (dv / b)**2
    phi = exp(-fac) / (b * sqrtPi)
...
! Make a 'ray' with a random position - then use it to update intensity and opacity
  subroutine getRay(grid, fromOctal, fromSubcell, position, direction, ds, phi, i0, thisMolecule,fixedrays,itransdone)
    ...
    logical :: itransdone(50)
    logical :: realdust
    real(double),save :: BnuBckGrnd(50)
...
!!! quasi random code!!!
    ...
    if(firsttime) then
       ...
       do itrans = 1, thisMolecule%nTrans
          ...
          transArray(iTrans,1) = thisMolecule%iTransUpper(iTrans)
          transArray(iTrans,2) = thisMolecule%iTransLower(iTrans)
       enddo
...
!       endif
    ...
    if (inOctal(grid%octreeRoot, currentPosition)) then ! check that we're still on the grid
       ...
       phi = phiProf((endprojVel+projvel)/2.d0, thisOctal%microturb(subcell))
    else
       phi = phiProf(projVel, thisOctal%microturb(subcell)) ! if fell off grid then assume phi is unchanged
    endif
    ...
    do while(inOctal(grid%octreeRoot, currentPosition))
       ...
       nTau = min(max(2, nint(dvAcrossCell * 20.d0)), 100) ! selects dVacrossCell as being between 0.1 and 10 (else nTau bounded by 2 and 200)
       distArray(1) = 0.d0
       ...
       do itrans = 1, nTrans
          ...
          iUpper(itrans) = transArray(iTrans,1)
          iLower(itrans) = transArray(iTrans,2)
...
!          if(isnan(dv)) then
!             write(*,*) "dv",dv, "deltaV",deltaV,"thisvel",thisvel,"direction",direction
!             stop
!          endif
...
!          alphatemp = hCgsOverfourPi! * phiProfVal !complete frequency redistribution

          do iTrans = 1, nTrans
          if(itransdone(itrans)) cycle
!             if(itransdone(itrans)) then 
!                write(*,*) "cycling", itrans
!                cycle
!             endif
             ...
             if(useDust) then             
                if(realdust) then
                   lambda = cspeed / thisMolecule%transfreq(iTrans) * 1e8 ! cm to Angstroms
...
!             write(*,*) "alpha gas", alphanu(1), "alpha dust", alphanu(2), "tau(gas)", alphanu(1) * distArray(2) * 1.d10, "tau(dust)", alphanu(2) * distArray(2) * 1.d10
             dTau = alpha * distArray(2) * 1.d10 ! distarray(2) is interval width & optical depth, dTau = alphanu*ds - between eqs (3) and (4)
!             etaLine =  !* thisMolecule%transFreq(iTrans)
             etaLine = hCgsOverfourPi * spontaneous(itrans) 
             
             if(usedust) then
                jnu = etaLine * phiProfVal + & !/ thisMolecule%transFreq(iTrans) + &
                  alphanu(2) * thisOctal%bnu(subcell,itrans) ! jnu, emission coefficient - equation 7
             else
                jnu = etaLine * phiProfVal !/ thisMolecule%transFreq(iTrans) + &
!                jnu = etaLine! * phiProfVal !/ thisMolecule%transFreq(iTrans) + & ! complete frequency distribution
             endif
...
!if(isnan(phiprofval))    write(*,*) "here",itrans,i0(itrans),jnu,alpha,phiprofval,spontaneous(itrans),dv,thisOctal%microturb(subcell)
!             if(tau(iTrans) .gt. 50) then
!                itransdone(itrans) = .true.
!             endif
...
!       do i = 2, nTau
!          distArray(i) = tval * dble(i-1) * OneOvernTau

!          startOctal => thisOctal
!          thisPosition = currentPosition + distArray(i)*direction
!          thisVel = amridVelocity(grid%octreeRoot, thisPosition, startOctal = startOctal, actualSubcell = subcell)  
!          dv = deltaV - (thisVel .dot. direction)
!          write(*,*) icount,dv*cspeed/1.d5,modulus(thisPosition)
!          icount = icount + 1
...
!             if(useDust) then
!                if(writeoutput) write(*,*) "Shouldn't be here"
!               lambda = cspeed / thisMolecule%transfreq(iTrans) * 1e8 ! cm to Angstroms
!                call locate(grid%lamArray, size(grid%lamArray), lambda, ilambda)
!                call returnKappa(grid, thisOctal, subcell, ilambda = ilambda, lambda = lambda, kappaAbs = kappaAbs)
!                alphanu(2) = kappaAbs * 1.d-10 !kappa already multiplied by density in amr_mod
!             else
!                alphanu(2) = 0.d0
!             endif

!             alpha = alphanu(1) + alphanu(2)
!             write(*,*) "alpha gas", alphanu(1), "alpha dust", alphanu(2)
!             dTau = alpha *  (distArray(i)-distArray(i-1)) * 1.d10 ! optical depth, dTau = alphanu*ds - between eqs (3) and (4)
!             etaLine =  !* thisMolecule%transFreq(iTrans)
!             etaLine = hCgsOverfourPi * thisMolecule%einsteinA(iTrans) * nUpper 
...
!             tau(iTrans) = tau(iTrans) + dtau ! contribution to optical depth from this line integral
!             if(tau(iTrans) .gt. 5) then
!                write(*,*) ntau, "Tau exceeded",tau(itrans),itrans,i
!                exit
!             endif
...
! sample level populations at logarithmically spaced annuli
  subroutine dumpResults(grid, thisMolecule, nRay)
    type(GRIDTYPE) :: grid
    type(MOLECULETYPE) :: thisMolecule
    integer :: nr
    real(double) :: r, ang
...
!       jnu = jnu / real(nr) ! normalised emission coefficient
       write(31,'(es11.5e2,4x,7(es12.6e2,tr2))') real(r*1.d10), real(pops(1:7)) ! real-time check that everything is fine
    enddo
    close(31)
...
! Does a lot of work - do more rays whilst problem not converged -            
  subroutine molecularLoop(grid, thisMolecule)
    ...
    use input_variables, only : blockhandout, tolerance,debug,lucyradiativeeq,geometry,lucyfilenamein,openlucy, amr1d!, restart ! ??
    ...
    type(OCTAL), pointer :: thisOctal
    integer, parameter :: maxIter = 100, maxRay = 41000000
...
18   format(a,tr3,8(a,tr5),a,6(tr1,a))
     ...
     if(restart) then
        ...
        call allocateMolecularLevels(grid, grid%octreeRoot, thisMolecule, .true.)
     else
        if(writeoutput) then
     ...
     if(.not. restart) then
        ...
        if(amr1d) call dumpresults(grid, thisMolecule, 0) ! find radial pops on final grid     
        if(Writeoutput) write(*,*) "Solved pops"
     endif
        ...
        do iOctal = 1, SIZE(octalArray)
           ...
           do subcell = 1, thisOctal%maxChildren
              if (.not.thisOctal%hasChild(subcell)) then
                 ...
                 write(11,*) &
                      "jnu",thisOctal%jnu(subcell,1),&
                      "bnu",thisOctal%bnu(subcell,3),&
                      "abundance",thisOctal%molAbundance(subcell),&
...
!                   do i=1,thisMolecule%nlevels
!                      if(isnan(thisOctal%molecularLevel(subcell,i))) write(*,*) "NO",i
!                   enddo

!                   if(.not. itransdone(itrans)) then
                      thisOctal%newMolecularLevel(subcell,:) = thisOctal%molecularLevel(subcell,:)
!                   else
!                      thisOctal%MolecularLevel(subcell,:) = thisOctal%molecularLevel(subcell,:)
!                      thisOctal%newMolecularLevel(subcell,itrans) = thisOctal%molecularLevel(subcell,itrans)
!                   endif
...
! Calculate convergence towards solution (per level)
     nTrans = thisMolecule%nTrans !/ 2 
     allocate(maxFracChangePerLevel(nTrans))
     ...
     call swapPops(grid%octreeRoot, maxFracChangePerLevel, avgFracChange, &
          convergenceCounter, convergencearray, grand_iter, nVoxels, iTransdone, fixedrays) ! compares level populations between this and previous levels 

     if(grand_iter .eq. 100) then
        if(writeoutput) write(*,*) "convergence",convergencearray(1:3,45,1:6)
        stop
     endif
...
! Write interesting things to a file ^

!          do i = 1, 8
!             if(real(convergenceCounter(4, i))/real(nVoxels) .gt. 0.99) then 
!		itransdone(i) = .true.
...
! If you think it's converged then test with the same number of rays to make sure
  ...
  subroutine calculateJbar(grid, thisOctal, subcell, thisMolecule, nRay, ds, phi, i0, iTrans, jbar, nPops)
    ...
    do iRay = 1, nRay
       ...
       etaLine = etaLine * thisOctal%nh2(subcell) * thisOctal%molAbundance(subcell) * nPops(iUpper)
       jnu = (etaLine) * phi(iRay)!/thisMolecule%transFreq(iTrans)

       if(useDust) then
          call returnKappa(grid, thisOctal, subcell, ilambda = ilambda, lambda = lambda, kappaAbs = kappaAbs)
          alphanu(2) = kappaAbs * 1.d-10 !* thisOctal%rho(subcell) - already done
          jnu = jnu + alphanu(2) * bnu(thisMolecule%transfreq(iTrans), dble(thisOctal%temperature(subcell)))
...
!     write(*,*) "jbar",jbarexternal,jbarinternal,sumphi
     if(isnan(jbarexternal)) then
  ...
  subroutine calculateJbar2(grid, thisOctal, subcell, thisMolecule, nRay, ds, phi, i0, iTrans, jbar, nPops)
    ...
    do iRay = 1, nRay
       ...
       alphanu(1) = hCgs*thisMolecule%transFreq(iTrans)/fourPi 
  ...
  function intensityAlongRay(position, direction, grid, thisMolecule, iTrans, deltaV) result (i0)
   ...
   currentPosition = position + (distToGrid + 5.d-2*grid%halfSmallestSubcell) * direction
    ...
    do while(inOctal(grid%octreeRoot, currentPosition))
       ...
       nLower = thisOctal%molecularLevel(subcell,iLower) * thisOctal%molAbundance(subcell) * thisOctal%nh2(subcell)
       nUpper = thisOctal%molecularLevel(subcell,iUpper) * thisOctal%molAbundance(subcell) * thisOctal%nh2(subcell)
       balance = (nLower * thisMolecule%einsteinBlu(iTrans) - nUpper * thisMolecule%einsteinBul(iTrans))
       ...
       nTau = min(max(5, nint(dvAcrossCell * 20.d0)), 200) ! ensure good resolution / selects dVacrossCell as being between 0.1 and 10 (else nTau bounded by 2 and 200)                                                                                                             
       !ntau = 10                                                                                                                                                                                                                                                                   
...
!          thisVel = startVel - (dble((i-1)/(nTau-1))) * (endvel - startvel) ! SPEEDUP 30/8/2007                                                                                                                                                                                    
          ...
          phiProfval = phiProf(dv, thisOctal%microturb(subcell))
          alphanu(1) = (hCgs/fourPi) * phiprofval !& ! thisMolecule%transFreq(iTrans)&                                                                                                                                                                                              
          !/thisMolecule%transFreq(iTrans)                                                                                                                                                                                                                                          
          ...
          etaLine = hCgs * thisMolecule%einsteinA(iTrans)! * thisMolecule%transFreq(iTrans)                                                                                                                                                                                         
          etaLine = etaLine * nUpper !* thisOctal%nh2(subcell) * thisOctal%molAbundance(subcell) * thisOctal%molecularLevel(subcell, iUpper)                                                                                                                                        
...
!          if(abs(deltaV) < 1.d-7 .and. i .gt. 10) write(*,*) "tau dust",dtau, "alphanu dust", alphanu(2),"itau",i
          ...
          if(useDust) jnu = jnu + alphanu(2) * bnu(thisMolecule%transfreq(iTrans), dble(thisOctal%temperature(subcell)))
...
!    i0 = i0 * (cSpeed**2 / (2.d0 * thisMolecule%transFreq(iTrans)**2 * kerg))
  ...
  function makeImageGrid(cube, unitvec, posvec, grid, thisMolecule, iTrans, deltaV, nsubpixels) result (imagegrid)
    ...
    imagebasis(2) = unitVec .cross. OCTALVECTOR(0d0,0d0,1d0) ! gridvector
...
!   call normalize(posvec)
    ...
    do j = 1,npixels ! raster over image
       if (j == 1) then 
       ...
       do i = 1,npixels
          index = (/i,j/)
          imagegrid(i,j) = PixelIntensity(cube,pixelside,viewvec,pixelcorner,imagebasis,grid,thisMolecule,&
...
!    if(subpixels .ne. nsubpixels) romberg = .true.
    ...
    do j = 1,subpixels ! this whole loop rasters across the imagegrid calculating the intensities at each position
       ...
       do i = 1,subpixels
          ...
          subpixelgrid(i,j) = i0
          totalPixelIntensity = totalPixelIntensity + subpixelgrid(i,j) 
          newposvec = newposvec + pixelbasis(2)
       enddo
    enddo

    subpixelgrid = subpixelgrid / dble(subpixels)**2
    totalPixelIntensity = totalPixelIntensity / dble(subpixels)**2
...
!  unitvec = randomunitvector()
  unitvec = OCTALVECTOR(sin(inc*degtorad),1d-10,cos(inc*degtorad))
! unitvec = OCTALVECTOR(1.d-10,0d0,1d0)

!  write(*,*) unitvec

  posvec = 300.d0 * grid%octreeroot%subcellsize * unitvec
...
! create an image with specified parameters  
  ...
  Tcmb = intensityAlongRay(1d10*unitvec, viewvec, grid, thisMolecule, iTrans,10.d5/cSpeed)
  write(*,*) "Tcmb",Tcmb
...
!  call plotDataCube(cube, filename, plotflux=.true.)
  ...
  call writedatacube(cube, 'MolRT.fits')
...
! SHOULD HAVE CASE(TELESCOPE) STATEMENT HERE
    ...
    write(message,*) "Observer Distance: ",gridDistance
    call writeinfo(message, FORINFO) 
    ...
    if(nv .ne. 0) then
       ...
       do iv = 1,nv
          ...
          fluxsum = intensitytoflux(intensitysum, dble(imageside), dble(gridDistance))
          ...
          fineweightedflux = intensitytoflux(fineweightedfluxsum, dble(imageside), dble(gridDistance))
       ...
       iv = 1
       do while (deltaV*cspeed/1.d5 .lt. maxVel)

          ddV = (maxVel-minVel)/100.
          cube%vAxis(iv) = deltaV * cSpeed/1.d5
          ...
          write(message,'(a,es11.4e1,tr3,a,f8.4,tr3,a,es12.4,a,es12.4,es12.4)') &
               "DELTAV(v/c):",real(deltaV)," V (km/s):",real(cube%vAxis(iv)), "Average Intensity:",intensitysum, &
               "nu*FLUX", (9.2e7/7.7e10)**2 * 8.9e10 * intensitysum, fluxsum * thisMolecule%transfreq(itrans)
...
subroutine createFluxSpectra(cube,thismolecule,itrans)
  ...
  use input_variables, only : gridDistance,molAbundance, beamsize
  character(len=80) :: filename
  type(datacube) :: cube
  type(moleculetype) :: thisMolecule
  real(double),allocatable :: fluxspec(:), inspec(:), antspec(:), weightedspec(:), brightnessspec(:)
  ...
  write(30,'(a,tr3,a,tr3,a,tr3,a)') "V (km/s)", "Intensity Wm-2Hz-1sr-1", "nu * Flux Wm-2","Weighted Flux" ! V channel, flux

  do i = 1, cube%nv
     fluxspec(i) = SUM(cube%flux(1:cube%nx,1:cube%ny,i))
     inspec(i) = SUM(cube%intensity(1:cube%nx,1:cube%ny,i))
     cube%flux(:,:,i)=cube%flux(:,:,i)*cube%Weight(:,:)
     ...
     write(30,'(f6.3,tr8,es13.4,tr6,es13.4,tr6,es13.4)') real(cube%vAxis(i)), inspec(i)/(1e3*(cube%nx**2)), &
                 thisMolecule%transfreq(itrans) * fluxspec(i) / 1e3, &
                 thisMolecule%transfreq(itrans) * weightedspec(i) / 1e3
  enddo
...
subroutine cubeIntensityToFlux(cube)
  ...
  dx = dx / (gridDistance*1.d-10) !  angluar width in radians
  cube%flux = cube%intensity * (dx**2) ! Flux through solid angle covered by one pixel in ergs/s
  !write(*,*) "DX", dx, dx**2, "gridist", griddistance
!  cube%flux = cube%flux * 1d23
end subroutine cubeIntensityToFlux

real(double) pure function IntensityToFlux(intensity,dx,distance) result (flux)
  
  real(double), intent(in) :: dx, intensity, distance
  
  flux = intensity * ((dx * 1.d10 / distance)**2) ! Flux through solid angle covered by one pixel in ergs/s
...
!     allocate(PixelPositionArray(npixels,npixels))
  ...
  subroutine getRay2(grid, fromOctal, fromSubcell, position, direction, ds, phi, i0, thisMolecule,fixedrays)
    ...
    type(OCTALVECTOR) :: position, direction, currentPosition, thisPosition, thisVel
    type(OCTALVECTOR) :: rayVel, startVel, endVel, endPosition
    real(double) :: alphanu(2), alpha, snu, jnu
    integer :: iLower , iUpper
    real(double) :: dv, deltaV
    integer :: i
    real(double) :: distArray(200), tval, cellEdgeInterval
    integer :: nTau
    real(double) :: nLower, nUpper
    real(double) :: dTau, etaline, kappaAbs 
    real(double), allocatable :: tau(:)
    real(double) :: dvAcrossCell, projVel, endprojVel, someval
    real(double) :: phiprofVal!(200)
    real :: lambda(50)
    integer :: ilambda(50)
    integer :: iCount

    real,save :: r1(3) !! quasi random number generators
    logical,save :: firsttime = .true.
    logical :: quasi = .false.
    real(double),save :: BnuBckGrnd(50)
    logical :: itransdone(50)
...
!       call random_number(r)
       ...
       direction = randomUnitVector() ! Put this line back if you want to go back to pseudorandom
!       deltaV = 4.3 * thisOctal%microturb(subcell) * (r - 0.5d0) ! random frequency near line spectrum peak. 
       deltaV = thisOctal%microturb(subcell) * (r - 0.5d0) ! random frequency near line spectrum peak. 
    endif
...
!       write(*,*) "b",phi
    ...
    if(useDust) then
       do iTrans = 1,nTrans
          lambda(itrans) = cspeed / thisMolecule%transfreq(iTrans) * 1e8 ! cm to Angstroms
          call locate(grid%lamArray, size(grid%lamArray), lambda(itrans), ilambda(itrans))
       enddo
    endif
...
!    write(*,*) "d"
          if(usedust) then
             call returnKappa(grid, thisOctal, subcell, ilambda = ilambda(itrans), lambda = lambda(itrans), kappaAbs = kappaAbs)
             alphanu(2) = kappaAbs * 1.d-10
          endif

          call findSubcellLocal(currentPosition, thisOctal, subcell)
          call distanceToCellBoundary(grid, currentPosition, direction, tVal, sOctal=thisOctal)
!     write(*,*) "e",currentposition,tval,thisOctal%label
          ...
          dvAcrossCell = ((startVel - rayVel).dot.direction) - ((endVel - rayVel).dot.direction) ! start - end
          dvAcrossCell = abs(dvAcrossCell / thisOctal%microturb(subcell))
!if(modulus(startvel) .ne. 0.)  write(*,*) "f",startvel,endvel,dvacrosscell,thisOctal%microturb(subcell)          
          nTau = min(max(5, nint(dvAcrossCell * 20.d0)), 100) ! selects dVacrossCell as being between 0.1 and 10 (else nTau bounded by 2 and 200)

!          allocate(PhiProfVal(nTau))

!          distArray = 0.d0

          cellEdgeInterval = tval / (nTau - 1.d0)
!    write(*,*) "g",cellEdgeInterval
          startOctal => thisOctal

!          do i = 2, nTau

!             distArray = distArray + cellEdgeInterval
!             thisPosition = currentPosition + distArray*direction   
!             thisVel = amrGridVelocity(grid%octreeRoot, thisPosition, startOctal = startOctal, actualSubcell = subcell)  
                
!             dV = deltaV - (thisVel .dot. direction)
!!             write(*,*) "h",dV
!             PhiProfVal(i) = phiProf(dv, thisOctal%microturb(subcell))
!!             write(*,*) "i",phiprofval(i)
!          enddo
...
!             write(*,*) "j",someval
!             if(someval .le. 0.d0) someval = 1d-30
             distArray(1) = 0.d0
             do i = 2,ntau
             ...
             someVal = (nLower * thisMolecule%einsteinBlu(iTrans) - &
                             nUpper * thisMolecule%einsteinBul(iTrans)) ! Equation 8


             distArray(i) = distArray(i) + cellEdgeInterval
             thisPosition = currentPosition + distArray(i)*direction   
             thisVel = amrGridVelocity(grid%octreeRoot, thisPosition, startOctal = startOctal, actualSubcell = subcell)  
...
!             write(*,*) "i",phiprofval(i)
                ...
                alpha = alphanu(1) + alphanu(2)
!                write(*,*) "k",alpha,alphanu(2)
                
                   dTau = alpha *  cellEdgeInterval * 1.d10 ! optical depth, dTau = alphanu*ds - between eqs (3) and (4)
                   
                   etaline = hCgsOverFourPi * thisMolecule%einsteinA(itrans) * nUpper

                   if(usedust) then
                      jnu = etaLine * phiProfVal + alphanu(2) * thisOctal%bnu(subcell,itrans)
                   else
                   ...
                   i0(iTrans) = i0(iTrans) +  exp(-tau(iTrans)) * (1.d0-exp(-dtau))*snu ! summed radiation intensity from line integral + 2nd term is local radiation field 
                      ...
                      snu = tiny(snu)
!             write(*,*) "l",i0(itrans),itrans,tau(itrans),dtau,snu
!                else
!                   write(*,*) "m!",i0(itrans),itrans,tau(itrans),dtau,snu,alpha,alphanu(1),phiprofval(i)
!                   write(*,*) "n",someval
                endif

                   tau(iTrans) = tau(iTrans) + dtau ! contribution to optical depth from this line integral
!             if(tau(iTrans) .gt. 10.) then
!                write(*,*) ntau, "Tau exceeded",tau(itrans),itrans,i
!                itransdone(itrans) = .true.
...
!       write(*,*) "OOOOOO"
    ...
    deallocate(tau)
  end subroutine getRay2

  subroutine getRay3(grid, fromOctal, fromSubcell, position, direction, ds, phi, i0, thisMolecule)
...
!             write(*,*) "alpha gas", alphanu(1), "alpha dust", alphanu(2)
             ...
             jnu = (etaLine/fourPi) * phiProf(dv, thisOctal%microturb(subcell)) / thisMolecule%transFreq(iTrans) + &
                   alphanu(2) * bnu(thisMolecule%transfreq(iTrans), dble(thisOctal%temperature(subcell))) ! jnu, emission coefficient - equation 7

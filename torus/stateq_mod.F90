...
!     module procedure mnewt_stateq_v2
  end interface
...
!       alpha(maxLevels+1,maxLevels+1) = incr/(tmp*fac)
  ...
  ! This is the original version written by TJH.
...
!    levels = boltzsaha(allLevels,Ne,T)  ! This is ok for elemental function...
  ...
  real(double) function rate_alpha_mn(m, n, Hnu1, nuArray1, nNu1, Hnu2, nuArray2, nNu2, &
    ...
    if (.not. grid%adaptive) then
       write(*,*) "Error:: grid%adaptive option not yet implmented in [stateq_mod::rate_alpha_mn]."
       write(*,*) "        If you need this option see equation14() and equation8() in this module."
       stop
    end if
    if(LyContThick) then
       write(*,*) "Error:: LyContThick option not yet implmented in [stateq_mod::rate_alpha_mn]."
       write(*,*) "        If you need this option see equation14() and equation8() in this module."
       stop
    end if
...
!          rate_alpha_mn = 0.0d0
          ...
          C_mk = cikt(m,real(thisOctal%temperature(thisSubcell),kind=db))
          rate_alpha_mn = -1.0d0*(Gm+Ne*C_mk)
          return
       else
       ...
       ! 
          ...
          if (n<m) then
             ! rates between n-th and its lower levels
             rate_alpha_mn =  bEinstein(n,m)*b_cnm*Inu + Ne*c_nm
          elseif (n>m) then
  ...
  real(double) function rate_beta_m(m, Hnu1, nuArray1, nNu1, Hnu2, nuArray2, nNu2, &
    ...
    if (.not. grid%adaptive) then
       write(*,*) "Error:: grid%adaptive option not yet implmented in [stateq_mod::rate_beta_m]."
       write(*,*) "        If you need this option see equation14() and equation8() in this module."
       stop
    end if
    if(LyContThick) then
       write(*,*) "Error:: LyContThick option not yet implmented in [stateq_mod::rate_beta_m]."
       write(*,*) "        If you need this option see equation14() and equation8() in this module."
       stop
    end if
    ...
    if (m == maxLevels+1)  then
       ...
       sum_Nm_LTE = 0.0d0
       do j = maxLevels+1, maxLevels+3
          Nm_LTE = boltzSaha(j, Ne, dble(thisOctal%temperature(thisSubcell)))
    ...
    do j = 1, maxLevels
       ...
       if (j < m) then
          ! rates between n-th and its lower levels
          tot =  tot + (Nn*bEinstein(j,m)-Nm*bEinstein(m,j))*b_cnm*Inu  &
               - Nm*aEinstein(m,j)*b_nm + Ne*(Nn*C_nm-Nm*C_mn)
       elseif (j > m) then
          ! rates between n-th and its  levels
          tot =  tot - (Nm*bEinstein(m,j)-Nn*bEinstein(j,m))*b_cnm*Inu  &
               + Nn*aEinstein(j,m)*b_nm + Ne*(Nn*C_nm-Nm*C_mn)
       end if
...
!      print *, n_iteration, 'trials' 
  ...
  real function approxIonization(Tr, Te) result (f)
    real :: tr , te

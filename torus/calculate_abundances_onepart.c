...
static void PrintFinalOutput(realtype *abundance, int nspec);
...
int calculate_abundances_onepart_(realtype *abundance, realtype *rate, realtype *density,
				  realtype *temperature, int *nspec, int *nreac, int nelectIn)
{
  ...
  realtype reltol = 1.e-8; /*!chemistry_module_mp_relative_abundance_tolerance_;*/
  realtype abstol = 1.e-30; //!chemistry_module_mp_absolute_abundance_tolerance_;
...
//#endif
      if (flag == 0) {
        if (nfails > 0) {
          fprintf(cvoderr, "Call to CVODE successful: t = %8.2le yr, # steps = %ld\n\n", t/seconds_in_year, mxstep);
#ifdef DEBUG_ABUND
          printf("Call to CVODE successful: t = %8.2le yr, # steps = %ld\n\n", t/seconds_in_year, mxstep);
#endif
        }
      }
      else {
        ...
        if (flag == -1) {
          if (nfails < 5) {
            fprintf(cvoderr, " Doubling mxstep and continuing the integration.\n\n");
            flag = CVodeSetMaxNumSteps(cvode_mem, mxstep);
            if (check_flag(&flag, "CVodeSetMaxNumSteps", 1)) status = 1;
	  }
          else {
            ...
            fprintf(cvoderr, "-----------------------------------------------\n\n");
#ifdef DEBUG_ABUND
            printf("ERROR! Fifth failure - aborting integration\n\n");
            printf("-----------------------------------------------\n");
            printf("CVODE Parameters:\n\n");
            printf("Before:\n");
            printf("flag   = %d\n", 0);
            printf("t      = %8.2le yr\n", start_time);
            printf("tout   = %8.2le yr\n", end_time);
            printf("dt     = %8.2le yr\n\n", end_time-start_time);
            printf("After:\n");
            printf("flag   = %d\n", flag);
            printf("t      = %8.2le yr\n", t/seconds_in_year);
          ...
          if (nfails < 5) {
            fprintf(cvoderr, "  Attempting to continue the integration using the same parameters.\n\n");
          }
          else {
            /*fprintf(stderr, " WARNING! CVODE solver failed: flag = %d\n", flag);*/
            fprintf(cvoderr, "ERROR! Fifth failure - aborting integration\n\n");
          ...
          fprintf(cvoderr, "-----------------------------------------------\n\n");
#ifdef DEBUG_ABUND
          printf("ERROR! CVODE failed with the following parameters:\n\n");
          printf("-----------------------------------------------\n");
          printf("CVODE Parameters:\n\n");
          printf("Before:\n");
          printf("flag   = %d\n", 0);
    ...
    /*for (i=212; i<215; i++) {
     ...
     printf ("end_time= %E \n",end_time);
     printf ("nelect= %i \n",nelect);
     printf ("rate(n=%i,nreac=%i)= %E \n",n,*nreac,rate[n*(*nreac)]);
     printf ("density(%i)= %E \n",n,density[n]);
     printf ("temperature(%i)= %E \n",n,temperature[n]);
     printf ("n %i ,abundance( %i )= %E \n",n,i,abundance[n*(*nspec)+i]); 
     printf ("\n");
    }
    printf ("\n\n");*/
    ...
    for (i = 0; i < neq; i++) {
#ifdef LOG_ODES
      abundance[i] = exp(data[i]);
#else
      abundance[i] = data[i];
#endif
      if(abundance[i] < abstol) abundance[i] = 0;
    }
    abundance[nelect] = user_data->x_e;
...
static void PrintFinalOutput(realtype *abundance, int nspec)
{
  ...
  printf("\n");
    ...
    for (i = 0; i < nspec; i++) {
      ...
      printf("Final y(%2d) =%12.5Le\n", i+1, abundance[i]);
      /* printf("%12.5Le", abundance[n*(nspec)+i]); */
#elif defined(SUNDIALS_DOUBLE_PRECISION)
      printf("Final y(%2d) =%12.5le\n", i+1, abundance[i]);
      /* printf("%12.5le", abundance[n*(nspec)+i]); */
#else
      printf("Final y(%2d) =%12.5e\n", i+1, abundance[i]);

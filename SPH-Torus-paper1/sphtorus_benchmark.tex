...
\begin{abstract}
  ...
  induced resulting in scale height enhancements which intecept
  ...
  ultimately reaches a state of simulatanous hydrostatic and radiative
  equilibrium.
...
This paper presents a results from coupling the {\small{TORUS}}
radiative transfer code \citep{harries_2000} with the SPH (Smoothed
Particle Hydrodynamics) code of \cite{bate_2009}.  {\small{TORUS}}
...
cicumstellar disc benchmark (see
...
in Section~\ref{section:sph_disc}.
...
increased and reaches a very low level with $10^8$ particles. 
...
$10^8$ particles but is less well represented with $10^6$ or $10^7$
...
all four cases the total disc mass is well represented and the long
...
siz different times during the simulation. Figure~\ref{fig:frame1}
...
frame (Fig.~\ref{fig:frame2} is shortly after the raditive transfer
...
height enhancements an in each case there is cooling behind the
enhancement. The scale height fluctations are able to affect the
...
the solid line is the vertically isothermal disc shown in
Fig.~\ref{fig:frame1} and the dotted line is the disc at the final
time step (Fig.~\ref{fig:frame6}).  Away from the inner and outer
edges of the disc the surface density is well represented by a power
law profile as a function of radius.
\begin{figure}
...
modelling the SED using this method seems to be considerably more
demanding than accurately representing the temperature structure
within the disc and temperature errors can be reduced to a managable
level with a realistic number of SPH particles in the disc. 

The combined SPH-radiative transfer code was able to evolve a
circumstellar disc to a state of radiative and hydrostatic
equilibrium. As the disc settles towards an equilibrium state, vertical
motions are seen which give the appearance of an outward propagating
scale height fluctuation. This is a vertical motion which decays more
rapidly at smaller radii and is seen whether radiative transfer is
included or not. One consequence of these fluctuations is that they
can intecept radiation from the central star which can then be
vertically transported into the mid-plane of the disc. Even thought
the fluctuations are of a small amplitude they exert an influence on

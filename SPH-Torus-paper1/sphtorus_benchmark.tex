...
\section{SED generation and temperature distribution}
...
particles and the enhanced grid resolution described above. SEDs are
plotted in Fig.~\ref{fig:n_part_sed_013} for a viewing angle of 12.5
degrees and Fig.~\ref{fig:n_part_sed_077} for a viewing angle of 77.5
degrees. The density distribution in the central region is plotted in
...
large. With $10^7$ particles there
is more material close in to the source but the gap itself is not
represented and the source is obscured. Only with $10^8$ particles is
a gap of approximately the correct size present. 

Figure~\ref{fig:n_part_tem_diff} shows fractional errors in the
temperature distribution for different numbers of particles. {\small{TORUS}}
calculates the temperature distribution using a high resolution 2D
geometry, in order to determine the correct temperature as a function
of cylindrical polar $r$ and $z$ co-ordinates. This is then
compared to the temperature from the SPH disc for each SPH
particle. Positive values of the fractional error indicate that the
SPH disc is hotter than the expected value.
\begin{figure*}
...
\end{figure*}
When using $10^5$ particles to represent the disc there is a
too much heating in the disc mid plane and insufficient heating
further out of the mid plane. The effect is reduced when $10^6$
particles are used but is only reduced to a low level with $10^8$
particles. The representation of the central region of the disc not
only has a significant impact on the SEDs but also on the temperature
distribution within the disc. 

To allow a more quantitative comparison the fractional difference in
mid-plane temperature, compared with the results of
\cite{pascucci_2004}, is plotted in
Fig.~\ref{fig:n_part_tem_diff_midpane}.
\begin{figure*}
...
\end{figure*}
In each case the particles are binned into 1~AU bins and the particle
closest to the mid-plane in that bin is found. The mid-plane
temperature from \cite{pascucci_2004} is linearly interpolated to the
cyclindrical radius of the particle and the fractional temperature
error is calculated. 
...
years). This time interval was chosen to allow the hydrodynamics to
respond to the radiative heating and achieve a suitable balance
between the hydrodynamics and the slower radiative transfer
calculation. 

...
Meanwhile dedicated radiation transfer (RT) codes  have progressed far beyond the diffusion approximation and now take into account a wide array of microphysical processes such as polychromatic radiation transfer incorporating polarization state, multiple scattering, Mie phase matrices etc. These sophisticated RT codes have been employed to model cores (e.g. \citealt{stamatellos_2005}), discs (reviewed by \citealt{dullemond_2007}) and even entire clusters \citep{kurosawa_2004}. The principle barrier to using these codes in RHD is that of  of computation time, but the increase in CPU power and the  adoption of parallelization mean that it is now feasible to conduct a full treatment of RT along with the hydrodynamics, thereby dropping the diffusion approximation.
...
in the accuracy of the SPH and radiative transfer codes individually so
we focus here on the accuracy of the mechanism by which they are
...
grid constructed from a mass per cell limit of $5\times10^{26}$g.  The
...
SED. SEDs were calculated using the {\sc torus} code for viewing
...
particles. {\sc torus} calculates the temperature distribution
...
hydrodynamics time steps (equivalent to every 2.4 years). The frequency
...
azimuthal cells, was used for the radiative transfer step. The AMR grid
...
within the disc and temperature errors can be reduced to a manageable

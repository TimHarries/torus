...
to benchmark the SPH component of the code here. Radiative transfer in
a circumstellar disc has been studied in detail by
\cite{pascucci_2004} and this benchmark is regularly used to test the
accuracy of the {\small{TORUS}} code. Consequently we can be confident
in the accuracy of the SPH and raditive transfer codes individually so
we focus here on the accuracy of the mechansim by which they are
coupled. Hence the benchmark case presented here is a ``static''
...
feasible and we expect the combined code to be used in applications
where the increased accuracy of the Monte-Carlo radiation calculation,
over faster alternatives, is of importance. 
...
enhancement as the scale height fluctations are able to affect the
...
support resulting in a smaller scale height.

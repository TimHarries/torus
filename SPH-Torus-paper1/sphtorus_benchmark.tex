...
the Eulerian grid desciption.  The representation on grid should
...
an accurate representation of the disc mass.
...
free surface, as described by \cite{price_2007}. 

%The method used to
%construct an AMR grid from SPH particles is described in more detail
%in \cite{rundle_2009}.
...
original SPH representation whereas the second condition should be
better at representing gradients in density which are likely to be
important for radiative transfer calculations. 
...
\label{eqn:cart_cell}
\end{equation}
where $dM_{\rm{cart}}$ is the mass of a cell in a cartesian grid and
$\rho_{\rm{cell}}$ is the density of the cell. 
%
In a cylindrical polar grid the mass of a cell is given by
\begin{equation}
dM_{\rm{cyl}} = \rho_{\rm{cell}} \frac{2 \pi r dr dz}{n_{\rm{azi}}}
\end{equation}
if there are $n_{\rm{azi}}$ uniform azimuthal cells. If the 
...
\begin{equation}
dl_{\rm{cyl}} = \left( \frac{n_{\rm{azi}} dM_{\rm{cyl}}}{2 \pi r
...
\begin{equation}
dM_{\rm{cyl}} = \frac{2 \pi r}{n_{\rm{azi}}} \rho^{1/3} dM_{\rm{cart}}^{2/3}
\end{equation}
...
enhanced resolution in regions of density contrast needs to be
combined with a mass per cell limit to ensure a robust total mass
representation.
...
Having investigated the procdure for setting up the radiative transfer
...
grid contructed from a mass per cell limit of $5\times10^{26}$g.  The
mass per cell limit was chosen in preference to the density contrast
limit to give the highest resolution near the central region of the
disc, which we expect to be most important for determining the
SED. SEDs were calculated using the {\small{TORUS}} code for viewing
...
below 1~$\mu\rm{m}$. The density distribution in the central region of
...
1AU and secondly the smoothing lengths of the particles are larger
than 1AU. In order to correctly represent the central gap there needs
...
inclination angle SED is dominated by emission from the central
star. This region is well represented with $10^5$ and $10^8$ particles
but is less well represented with $10^6$ or $10^7$ particles where the
central star is obscured. At the longest wavelengths the disc is optically
thin and the SEDs will be well modelled provided the total disc mass
is accurately represented. In all four cases the total disc mass is
well represented at the long wavelength SED matches the benchmark
results. At intermediate wavelengths the SED is sensitive to the
details of the disc representation. Accurately modelling the SED from
...
A radiative transfer calculation is performed after every four
hydrodynamics time steps (equivalent to every 2.4 years). The freqency
with which the radiative transfer is calculated is determined by the
sound crossing time in the inner regions of the disc as this is the
time scale over which the scale height in this region will vary and
effect the transfer of radiation into the outer parts of the disc. 

A cylindrical polar grid, with adaptive $r$ and $z$ cells and 16
azimuthal cells, was used for the raditive transfer step. The AMR grid
...
$R_{\rm{gap}}=1.0~\rm{AU}$ which corresponds to the accretion radius
...
resolved. The first box of extra refinement has size $10^{14}~\rm{cm}$
with cells no larger than $10^{13}~\rm{cm}$ and the second box has
size $4\times10^{13}~\rm{cm}$ with cells no larger than
$4\times10^{12}~\rm{cm}$.  A real circumstellar disc will be truncated
by the stellar magnetosphere so there is a physical justification for
enforcing a low density region around the star.

\subsection{Results}

Plots of the disc internal energy are shown in Fig.~\ref{fig:log_u} at siz
different times during the simulation. Figure~\ref{fig:frame1} shows
the initial state before the radiative transfer is switched on. The
disc is vertically isothermal. The second frame (Fig.~\ref{fig:frame2}
is shortly after the raditive transfer has been swithced on. The inner
disc has collapsed and there is heating seen in parts of the disc
exposed to radiation from the central star. In Fig.~\ref{fig:frame3} a
scale height enhancement is present in the disc at a radius of
approximately $3\times10^{15}~\rm{cm}$ which shadows the region
immediately behind causing it to cool. Figure~\ref{fig:frame4} shows
several such scale height enhancements an in each case there is
cooling behind the enhancement. The scale height fluctations are able
to affect the mid-plane temperature due to vertical transport of
radiation. In Fig.~\ref{fig:frame5} the inner region of the disc has
settled and the scale height enhancements are seen only in the outer
disc and by the last frame (Fig.~\ref{fig:frame6}) the disc has
reached a steady state in hydrostatic and radiative equilibrium. 
\begin{figure*}
  ...
  \caption{Particle plots showing internal energy in cyclindrical
    polar co-ordinates. Axis units are cm.}
  \label{fig:log_u}
\end{figure*}

The surface density of the disc is shown in Fig.~\ref{fig:sigma} where
the solid line is the isothermal disc and the dotted line is the disc
at the final time step.  Away from the inner and outer edges of the
disc the surface density is well represented by a power law profile as
...
height. The solid line is the isothermal disc and the dotted line is
the disc at the final time step.
\begin{figure}
...
is plotted in Fig.~\ref{fig:mid_tem}. The solid line is the isothermal
disc and the dotted line is the disc at the final time step.
\begin{figure}

...
occultation of the flow  by the inner part of the accretion disc and
by the stellar photosphere, while inverse 
...
be absorbed by the gas locally, and re-emitted as optical and UV
...
find the line shapes of H$\alpha$ do not change significantly.
...
on a adopted mass-accretion rate. The continuum emission from the hot
...
\caption{Summary of the reference classical T~Tauri star model
  ...
  effect of line-broadenings more clearly (c.f. Section~\ref{sub:model-acc}). }
...
Although the model of \citet{martin:1996} seems to be more comprehensive
than that of \citet{hartmann:1994}, \citet*{muzerolle:1998} found that
the line profile models computed according to the temperature
structure of the former do not agree with observations while they do
have much better agreement when the temperature of the latter model was
used.  The temperature structure of the magnetosphere is still a big
uncertainty in the accretion model; however, for a heuristic reason,
we adopted the model of \citet{hartmann:1994}.
...
distance for the following two reasons: (1)~to simulate more
\emph{collimated} wind (toward the symmetry axis, and (2)~to be more
consistent with the fraction of H$\alpha$ profiles affected by wind
absorptions in observations.

Since the stream lines in our simple disc wind are represented by
straight lines, the wind never bends toward the symmetry axis ($z$-axis)
as the distance from the disc increases unlike the
model of \citet{blandford:1982}. We therefore used a larger
launching angle to simulate the stream lines in the far field (at a larger
distance from the disc plane) in their model.  This is an intrinsic
problem with approximating the stream lines with straight lines. A
better model is perhaps to compute the field lines using
\citet{blandford:1982} model as done so in \citet{alencar:2005}.
Also, in our mode, if the launching angle is too small
($<60^{\circ}$), the wind absorption feature would be not been seen
or would be 
very small for profiles computed with an inclination angle
$i<60^{\circ}$.  If the launching angle is $60^{\circ}$ at the inner
most part of the wind, and the inclination 
angle (measured from the symmetry axis, $z$-axis) is less than $60^{\circ}$,
little (wind) absorption is expected, as demonstrated in Fig.~\ref{fig:discwind_d_effect}.
The situation will however change if the field/stream lines are not
straight and bending towards the symmetry axis as the wind travel
higher above the disc.  Since more than 50~per~cent of observed
H$\alpha$ profiles (e.g. \citealt{reipurth:1996};
\citealt{alencar:2000}) are affected by wind absorption, it was 
necessary for our model to used the launching angle (at the inner most
part) greater than $60^{\circ}$ to be more consistent with observations;
although this would mean that the launching angle condition of
\citet{blandford:1982} is violated for wind from a very inner part
of the disc.
...
of \citet{blandford:1982} in all parts of the disc wind zone in our model.  These
models showed that the wind absorption and emission features weaken,
but overall morphology dependence of the profiles on the wind
temperature rate and the wind acceleration rate is similar to what we
found in the models shown in Figs.~\ref{fig:atlas_discwind_only} and
\ref{fig:atlas_hybrid}.  The minimum inclination angle at which the
wind absorption becomes important is larger than that of the models
with a larger source displacement ($d = 22\, R_{*}$) as expected.
...
flux in the absorption trough appears to remain at same positions.  On the other
...
significant amount of variablity, which is in some cases induced by
the rotational motion of an inclined magnetosphere
(e.g.~\citealt{johns:1995}; \citealt{bouvier:2003}), we restrict our
discussion in this section for the cases of axi-symmetric geometries.
...
inclination models.
In addition to the models shown in Appendix~\ref{sec:appendix}, we
have run a set of hybrid models (not shown here) with a typical
mass-accretion rate for CTTS i.e. $3\times10^{-8}
\mathrm{M_{\sun}\,yr^{-1}}$ (\citealt{gullbring:1998};
\citealt{calvet:2004}) using $i=10^{\circ},55^{\circ},80^{\circ}$ with
combinations of $\beta = 0.2, 0.5, 1.0, 2.0$ and
$T_{\mathrm{wind}}=7000, 8000, 9000, 10000$~K. Based on these models,
we found that Type~III-B profiles can be seen for at least in three
models with fast accelerating and high wind temperature at moderate
inclination angle i.e. $i = 55^{\circ}$ with $\left ( \beta,
T_{\mathrm{wind}} \right ) = \left ( 0.2, 10000\,\Kelvin \right )$,
$\left ( 0.5, 9000\,\Kelvin \right )$ and $\left ( 0.5, 10000\,\Kelvin
\right )$.  A possible reason for not being able to find Type~III
profiles in lower mass-accretion rate models (e.g. in
$\dot{M}_{\mathrm{acc}} =
1.0\times10^{-8}\,\mathrm{M_{\sun}\,yr^{-1}}$ models) is perhaps due
to a wrong radial dependence ($p$ in
equation~\ref{eq:discwind-massloss-w}) of the mass-loss rate per unit
area adopted in our models.  This dependence shall be examined more
carefully in future.
...
relaxed if the axis of the magnetosphere is tilted with respect to the
rotational axis since the line-of-sight to an observer from the
accretion hot rings will pass through a different part of the
magnetosphere as a rotational phase changes (as it precesses around
the rotational axis).
...
\begin{figure*}
  ...
  \caption{The dependence of the H$\alpha$ model profiles on the
    ...
    The wind mass-loss rates used in both models are
    $\dot{M}_{\mathrm{wind}}=10^{-8}~\mathrm{M_{\sun}~yr^{-1}}$ . 
...
surface, the mass-flux conservation, and using the beta velocity law
...
is same as the mass-loss rate used in the disc-wind magnetosphere
...
{Pudritz} R.~E., {Banerjee} R., 2005, in Proc.~IAU Symp. Massive star
birth: A crossroads of Astrophysics, Cesaroni, R., Felli, M.,
Churchwell, E., Walmsley, M., eds, Cambridge Univ.\, Press, Cambridge,
225, p.~163

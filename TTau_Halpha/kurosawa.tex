...
in Fig.~\ref{ig:config_discwind} (region~2), the innermost radius ($R_{\mathrm{mi}}$)
...
section~\ref{sub:model-disc-wind}. This type of outflow 
...
its density increasing toward the equatorial plane. 

GEOMETRY? REFERENCE MODEL?
...
Table~\ref{tab:std_discwind_parameters}.
...
$T_{\mathrm{wind}}$.  Figure~\ref{fig:atlas_discwind_only} shows profiles computed at  $i=55^{\circ}$
...
disc-wind temperature ($T_{\mathrm{wind}}$) and the wind acceleration
parameter ($\beta$) ranges used for the bipolar wind models
(c.f. Figures~\ref{fig:atlas_wind_model} and
\ref{fig:atlas_disc_wind_acc_model}).  

  The figure shows that the
wind emission increases as $\beta$ and $T_{\mathrm{wind}}$ increase. The
...
the models with smaller $\beta$ and $T_{\mathrm{wind}}$, the line
emission from the magnetosphere dominates, while this situation reverses for the
models with larger $\beta$ and $T_{\mathrm{wind}}$. The model with
...
computed with limited ranges of  $T_{\mathrm{wind}}$ and $\beta$, the
...
(section~\ref{sub:model-disc-wind}; Figure~\ref{fig:config_dipolar}), and

...
profiles, and to any other lines affected by the stellar wind,
...
the out-flowing wind component is defined only in the outside of the
...
Two different types of the wind configurations are considered: (1)~the
bipolar wind model which resembles the density structure of the MHD
simulation by \citet{krasnopolsky:2003} in \em large scale \em ($\sim
100$~au), and (2)~the disc-wind model which resembles the density of
the same MHD simulation in \em small scale \em ($\sim 10$~au).  While
the wind density is more concentrated toward the polar directions in
(1), it is more concentrated toward the equatorial direction in
(2). The following describes each model in details.

\subsubsection{Bipolar Wind}
...
profile can be reproduced with the disc-wind, magnetosphere hybrid

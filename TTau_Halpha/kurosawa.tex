...
We present  radiative transfer models of classical T~Tauri stars, and
investigate the formation of H$\alpha$ line emission 
in order to understand their complex circumstellar environments, and to explain
the wide variety of line profiles seen in observations. To
overcome the limited applicability of the radiative transfer models
with magnetospherical accretion alone to fitting observed H$\alpha$
...
show direct evidence for the presence of bipolar and monopolar
...
disc both above and below the disc, suggesting the presence
of the bipolar outflows. On a larger scale, \emph{HST} observations
of HH30 \citep{burrows:1996} show the jet traced to within $\lesssim30$~AU
...
absorption components in at least one of the Balmer lines and Ca~K (most
commonly in H$\alpha$).  

In the currently favoured model of accretion in CTTS, the
accretion disc is disrupted by the magnetosphere of stars which
...
by the evidence that CTTS have relatively strong ($\sim 10^{3}\mathrm{G}$)
magnetic field (e.g., \citealt{johns-krull:1999};
\citealt{guenther:1996};  \citealt{symington:2005b})
and by radiative transfer models which reproduce the observed
...
the flow by the stellar disc, and the redshifted absorption component at the
...
overwhelming observational evidence for outflow (mentioned above)
in the CTTS profiles suggests that this model is only a part of the
complete picture. Clearly, a modification to include an out-flowing
wind/jet flow is necessary if one wishes to predict the mass-accretion
rate and the mass-loss rate of CTTS by modelling their emission profiles
(e.g. H$\alpha$).

Prior to the magnetospherical paradigm, many alternative models had
...
\citealt*{calvet:1992b}; \citealt{kwan:1995}).  Recent work by
\citet{alencar:2005} showed the observed 
H$\alpha$, H$\beta$ and Na~D lines of RW Aur are better be produced
by the radiative transfer model which includes a collimated disc-wind
arising from near the inner edge of the accretion disc. \citet{malbet:2005}
considered a combination of a spherical wind and an accretion disc
...
originated from the star can cause significant spin-down torque on
...
of emission line profiles seen in the observation, and will help
to construct more comprehensive circumstellar models (e.g. via MHD
simulations) of T~Tauri stars. Using the model results, we examine
...
contributes to the formation of H$\alpha$, the model space is divided
into four different regions: (1)~a central continuum source, (2)~the magnetospherical
accretion flow, (3)~the wind outflow, and (4)~the accretion
disc. Fig.~\ref{fig:config_dipolar} 
depicts the relative location of the regions in the model space. In
all regions, the density is symmetric around the z-axis. The innermost radius of
...
surface of the star. For simplicity,
the collimated bipolar wind is defined only in the outside of the
...
wind resembling the density structure of the MHD simulation by \citet{krasnopolsky:2003}
in large scale ($\sim 100$~AU) in section~\ref{sub:wind_jet}, and
(2) that in small scale ($\sim 10$~AU) in section~\ref{sub:Alternative-model-disc-wind}. 
...
y-axis are into the paper, (2)~the magnetospherical accretion flow,
...
inner radius of the accretion disc. From the inner most part of the
...
$T_{\mathrm{ph}}=4000\,\mathrm{K}$ and $\log g_{*}=3.5$ (cgs) is used
to find the photospheric continuum at the H$\alpha$ wavelength. The
...
continuum source is considered. As the in-falling gas 
becomes close to the stellar surface, it is decelerates in a strong
...
the collimated MHD disc-wind model of \citet{krasnopolsky:2003} on a 
large scale ($\sim 100$~AU) , and an alternative wind model which
resembles the model of \citet{krasnopolsky:2003} in small scale ($\sim
10$~AU)
...
wind. For the radius ($r/R_{\mathrm{mo}} < 10$, both velocity and
...
plots. Beyond $r/R_{\mathrm{mo}}= 100$, little difference is seen in
the polar density with different $\beta$ values.}
...
density) by using an iterative Monte Carlo radiative transfer technique
(c.f.~\citealt{walker:2004}), we find it to be too time consuming
...
\subsection{Alternative model: the disc-wind model}
...
coordinates $\left(x,y,z\right)$ -- the y-axis are into the paper,
(2)~the magnetospherical accretion flow, (3)~the collimated wind/jet
outflow, and (4)~the geometrically thin (but opaque) accretion disc. The
...
star. By changing the value of $d$, the degree of the wind collimation
is controlled. The density distribution is symmetric around the
z-axis. }
...
The wind model presented here is intended to mimic the collimated MHD disc-wind
calculations of \citet{krasnopolsky:2003} in small scale ($\sim 10$~AU).
...
degree of the wind collimation, (3)~the velocity gradients, and (4)~the
...
\caption{Dependency of the disc-wind density and velocity on the wind
...
($V_{\theta}$), which is independent of $\beta$
(c.f. equation~\ref{eq:discwind-toroidal-velocity}), is also shown in
the right panel for a comparison. The density is normalised to the
density $\rho_{0}$ at the wind launching point for $\beta = 1.0$ case.
The $v_{p}$ reaches the terminal velocity by 100~AU for all
$\beta$. In the far field ($l>10$~AU), the density is approximately
proportional to $\sim l^2$. Up to $l~\sim 10$~AU, the density is
smaller and the poloidal speed is larger for the wind with a larger
$\beta$ value. }
...
by one or more of the following: the magnetospherical accretion flow,
...
and restricted our models to be axi-symmetric. It was done in order
to enable us to explore rather large number of parameters (c.f.~section~\ref{sec:model-configuration}).
...
angle. An examples of how the AMR grid for the purpose of the radiative
transfer is constructed are presented in e.g. \citet*{wolf:1999},
\citet{kurosawa:2001a}, \citet{steinacker:2003} and \citet{kurosawa:2004a}.
...
consist of 14 bound state and continuum. Readers are refer to \citet{harries:2000}
for details. 
...
\citealt{reipurth:1996}) which cannot be explained by the infall velocity
of the gas along the magnetosphere alone. 

To implement the broadening mechanism into our previous
model (\citealt{symington:2005}), we follow the formalism adopted 
...
atmosphere model of \citet{kurucz:1979} as described in section~\ref{sub:Continuum-Source},
...
is zero. Since the disc mass of CTTS are rather small ($\sim 0.01\,\mathrm{M_{\sun}}$)
and low temperature ($<\sim 1600\,\mathrm{K}$), the continuum flux
...
profiles are computed with the magneospherical accretion flow using
...
\caption{H$\alpha$ model profiles computed with wide ranges of the
  ...
  accerelation rate ($\beta$). The profiles are computed with the wind
  ...
  the inclination ($i$) and the degree of collimination ($b$) are
  ...
  accerelation rate increases from left to right. The P~Cygni profile
  are seen for lower $T_{\mathrm{wind}}$ and lower $\beta$ (slow
  accerelation) models.  The postion of the absorption the component
  moves toward the line centre as $\beta$ increases. The effect of
  line broadening becomes more prominant for higher $T_\mathrm{wind}$
...
P~Cygni profile feature becomes prominant for the spherical wind model
...
disc, i.e. the continuumm flux level is unchanged.  The H$\alpha$
...
profile computed with only the magnetopsherical accretion (thick
...
profile computed with only the magensopherical accretion flow (dotted)
is overlayed. All the profiles are computed at the inclination angle
$i=55^{\circ}$. As $\beta$ becomes larger, the postion of the wind
...
$T_{\mathrm{dw}}=9000\,\mathrm{K}$ displays a small amount of the wind
...
profiles (\citealt{reipurth:1996}), which cannot be explained
...
accelreation rate ($\beta$) and the isothermal disc wind temperature
...
larger. The morphology of the profiles are smilar to that of
observations. }
...
\caption{The disc-wind, magnetoshere hybrid model. The same as
...
smaller $\beta$ and $T_{\mathrm{dw}}$ (the faster accerelation and the
...
and II-B types (see \citealt{reipurth:1996}) as the value of $\beta$
...
as the value of $\beta$ increases (the wind acceleration is slower).
...
} types of  H$\alpha$ profiles (\citealt{reipurth:1996}).
...
comes from the magnetosphere, and the profile shape is altered from a slightly
...
the disc-wind, magnetosphere hybrid model with a pole-on configuration.
Since there is no material in the polar region in the disc wind model,
...
be produced in similar physical conditions (e.g. in a slow accretion
and low temperature wind plus a magnetosphere). The main difference is
between the two is in their inclination angles. The former is more
likely to be seen at high inclination angles.
...
difficult to reproduce this type of profile since this can be achieve only
at narrow range of parameter space (very high inclination angle with
a disc wind model, $T_{\mathrm{max}}\sim8300\,\mathrm{K}$, and
$\dot{M}_{\mathrm{acc}}~10^{-8}\,\mathrm{M_{\sun}yr^{-1}}$).  
...
relatively fast moving gas in the accretion stream. Finally Type III-B
profile is very similar to Type II-B, but the wind in this type of
profile may be accretion faster than that in Type II-B.
...
\caption{Sample H$\alpha$ model profiles which charactersize the
classfication by \citet{reipurth:1996}. The combination of
...
are velocities in $\kmps$, and the vertical axes are nomalized
flux.}
...
Figure~\ref{fig:atlas_reipurth} and brief comments. The temepratures
...
angle. The profile computed at $i=10^{\circ}$(solid), $55^{\circ}$
(dot) and $80^{\circ}$ using 1.~the disc, bipolar-wind, accretion
hybrid model (left panel; c.f. section~\ref{sub:model-acc-disc-wind})
and 2.~using the disc-wind, magentosphere hybrid model (right;
c.f. section~\ref{sub:result-discwind-knigge}) are shown for a
comparison.  The parameters used for the magnetoshere are same for the
both models, i.e. $T_{\mathrm{max}}=7500~\mathrm{K}$ and
$\dot{M}_{\mathrm{acc}}=10^{-7}~\mathrm{M_{\sun}~yr^{-1}}$.  The
isothermal wind temperature and the wind accrelation parameter used
both models are $T=7000~\mathrm{K}$ and $\beta=1.0$ respectively.  All
the other parameters are same as in
...
configuration, it decreases as the incination angle increases for the
...
type. Using the diffrent combinations of the inclination ($i$), the
mass-loss to mass-accretion rate ratio ($\mu$) and the wind
acceralation rate ($\beta$), our radiativel transfer model was able to

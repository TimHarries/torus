...
of HH30 \citep{burrows:1996} shows the jet traced to within $\lesssim30$~AU
...
by the evidences that CTTS have relatively strong ($\sim 10^{3}\mathrm{G}$)
...
complete picture. Clearly, the modification to include the out-flowing
...
into four different regions: (1)~a central continuum source, (2)~magnetosphere
...
\caption{Basic Model Configuration. The system consist of four
components: (1)~the continuum source located at the origin $\left( o
\right)$ of the cartesian coordinates $\left(x,y,z\right)$ -- the
y-axis are into the paper, (2)~magnetospherical accretion flow,
(3)~bipolar wind outflow, and (4)~accretion disc. The density
...
accretion disc, the gas freely fall, moving along the magnetic filed
onto the surface of the star. For a computational simplicity, the
bipolar wind starts from just outside of the largest radius
...
shock, and heated to $\sim 10^{6}\,\mathrm{K}$ (with typical parameters).
...
large scale ($\sim 100$~AU) , and the alternative wind model which
...
$\boldsymbol{v}_{\mathrm{wind}}$, is consist of
...
structures on the wind acceleration parameter $\beta$. The radial
component of the wind velocity
(equation~\ref{eq:wind_velocity_radial_part}) in polar direction,
$\theta = 0$ as a function of radius is shown on left. The wind
density (equation~\ref{eq:wind_density_final} along polar direction as
a function of radius is shown on right. For all $\beta$ values, the
...
(2)~magnetospherical accretion flow, (3)~collimated wind/jet
outflow, and (4)~geometrically thin (but opaque) accretion disc. The
...
calculations of \citealt{krasnopolsky:2003} in small scale ($\sim 10$~AU).
...
is zero. Since the disc mass of CTTS are rather small ($\sim 0.1\,\mathrm{M_{\sun}}$)
...
\caption{The summary of H$\alpha$ equivalent widths from the
magnetospherical accretion flow models show in
...
\caption{Effect of the line broadening for H$\alpha$. The model
computed with the damping constant ($\Gamma$), described in
...
the $\Gamma=0$ model are not seen in the model with the broadening.  }
...
\caption{Dependecy of H$\alpha$ profiles on inclination ($i$).  The
...
\caption{H$\alpha$ model profiles computed with wide ranges of the
  ...
  axis) of the model increase from top to bottom, and the wind
...
to be explored, we initially adopt $\gamma=0.05$, $v_{\infty}=210\,\kmps$
...
since the collumn density of along the line of a sight of the observer
...
The inner and the outer radii of the accretion disc are $2.2R_{*}$
...
\caption{Comparison of the wind models with and without the
...
greatly reduce by the presence of the accretion disc compared to that
...
flux is in an arbitrary units and in logarithmic (base $10$) scale. }
...
the wind, the disc, and their combinations. The model configureations
...
\caption{Effect of the mass-loss rate to mass-accretion rate ratio
...
becomes stronger. In genaral, the absorption is too strong for the
...
the disc-wind model of \citet{knigge:1995} as briefly described in section~\ref{sub:model-disc-wind}.
...
\caption{The summary of the standard disc-wind model parameters used
...
models. For the wind mass-loss rate fixed at
...
accelerates slower and as the wind comes warmer.  Similar to the
bipolar wind models (Figure~\ref{fig:atlas_wind_mode}), the position
...
\caption{The disc-wind, magnetoshere hybrid model. Same as
Figure~\ref{fig:atlas_discwind_only}, but these models (soild) also
...
colder wind models), the emission from the wind domnates for the
models with models with larger $\beta$ and $T_{\mathrm{dw}}$ (the
slower accerelation and the hotter models). For a comparison, the
profile computed with only the magensopherical accretion flow (dotted)
is overlayed. The model profiles are very similar to the observed
profiles influenced by the wind outflow.}
...
summarised in Table~\ref{tab:classification}. The horizontal axises
are velocities in $\kmps$, and the vertical axises are nomalized
fluxes.}

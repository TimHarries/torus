...
\begin{figure}
  ...
  \begin{center}
    ...
    \includegraphics[%
      scale=0.3]{figures/overview3_alt.eps}
  ...
  \caption{The basic model configuration. The system consists of four
    ...
    inner radius of the accretion disc. From the innermost part of the
    accretion disc, the gas falls freely, moving along the magnetic field
    onto the surface of the star. The wind density is enhaced toward
    the polar directions to mimic the collimated wind structure of the
    magneto-hydrodynamical simulations (e.g.~\citealt{krasnopolsky:2003}). }
...
that of \citealt{krasnopolsky:2003} in large scale, $>10$~au). The
second model is based on the disc-wind paradigm, in which the density
of the outflow increases towards the equatorial plane. The difference
in latitudinal density structure between the two outflow models should
provide an indication as to whether line profiles alone can be used to
constrain the outflow geometry. In the following sections we present
details of the two outflow structures.
...
the wind are fixed as the reference values given in
...
reference values (Table~\ref{tab:std_parameters}).  The density is
...
\begin{figure*}
  ...
  \caption{H$\alpha$ model profiles computed with wide ranges of the
    ...
    disc). The `reference' model parameters (Table~\ref{tab:std_parameters}) are
...
\begin{figure}
  ...
  \caption{A comparison of the profiles computed with the magnetosphere,
    ...
    $\dot{M}_{\mathrm{acc}}=10^{-7}\,\mathrm{M_{\sun}\,yr^{-1}}$,
    $\dot{M}_{\mathrm{wind}}=10^{-8}\,\mathrm{M_{\sun}\,yr^{-1}}$,
    $T_{\mathrm{wind}}=9000\,\Kelvin$, and $\beta=0.5$. The profile for
